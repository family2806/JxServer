...
function main(itemindex)
	...
	str=
	{	"<#>一颗大白驹丸，使用后可获得<color=red>8小时<color>的离线托管时间。",
		"<#>使用大白驹丸/confirmlevel",
		"<#>我再考虑考虑/cancel",
...
function confirmlevel()
	str=
	{	"<#>因为你的等级低于<color=red>50级<color>，所以在离线托管状态内，你可以进行摆摊交易，但不会获得<color=red>经验值<color>。你确定要使用大白驹丸吗？",
		"<#>确定/use",
		"<#>我再考虑考虑/cancel",
...
function use()
	...
	if ((sparetime + AEXP_BIGCHANGE) > AEXP_MAXTIME) then
		...
		Say("你目前的大白驹丸离线经验托管剩余时间为 <color=red>"..hours.."<color>小时<color=red>"..minutes.."<color>分钟，而你的离线经验托管时间累计不可以超过<color=red>10000小时<color>，请你过一段时间再使用大白驹丸。", 0);
    else	
		...
		if (nresult == 1) then 
			...
			Say("使用成功，你剩余的大白驹丸离线托管时间为 <color=red>"..hours.."<color>小时<color=red>"..minutes.."<color>分钟",0);
