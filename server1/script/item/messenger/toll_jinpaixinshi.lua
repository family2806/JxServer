...
function main(itemindex)
	...
	if ( paramvalue >= 5 ) then
		Msg2Player("对不起，你的信使金牌已经使用过5次了，它将被销毁。")
	elseif ( nt_getTask(1206) < 4 ) then
		Msg2Player("对不起，你还没有领取到金牌信使的称号，无法使用信使金牌。请先去找驿官升级称号，谢谢。")
		return 1
	else
		if ( SetSpecItemParam(itemindex, 1, paramvalue+1) == 1 ) then
			SyncItem(itemindex)
			local Realvalue = 5 - GetItemParam(itemindex, 1)
			Msg2Player("你的信使金牌使用后可获得一小时的25点负抗。目前还可以使用"..Realvalue.."次。")
		end
...
function GetDesc( nItem )
	...
	szDesc = ""
	szDesc = szDesc..format( "你的信使金牌使用后可获得一小时的25点负抗。目前还可以使用"..Realvalue.."次。");
	return szDesc;
end