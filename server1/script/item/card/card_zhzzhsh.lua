...
function PickUp( nItemIndex, nPlayerIndex )
	...
	if(size < 2) then --
		Msg2Player("你得到一张执子之手卡，但是孑然一身的你无法开启此卡的能力。")
		PlayerIndex = oldPlyIdx
		return 0
	end
	if(size > 2) then
		Msg2Player("你得到一张执子之手卡，但是这么多人，你不知道和谁才能开启此卡的能力。")
		PlayerIndex = oldPlyIdx
		return 0
	end
	...
	if(otherSex == mySex) then
		Msg2Player("你得到一张执子之手卡，但是无法与同性玩家开启此卡的能力。")
		PlayerIndex = oldPlyIdx
		return 0
	end
	if(myCode == otherCode and myCode ~=0) then  --验证码相同
		if(myNum == otherNum) then --有情人，卡的数量一样。
			...
			otherNum = myNum
			Msg2Player("你得到一张执子之手卡，把你和"..otherName.."的缘份拉得更近了，到目前为止，你积累了"..myNum.."张执子之手卡。")
			AddOwnExp(myLevel * (20+0.1*myLevel)-10)
			SetTask(TID_CARDNUMBER,SetByte(GetTask(TID_CARDNUMBER),GETBYTE,myNum)) --回写信息
			PlayerIndex = otherIndex
			Msg2Player(myName.."得到一张执子之手卡，把你们的缘份拉得更近了，到目前为止，你积累了"..otherNum.."张执子之手卡。")
			...
			if(myNum > 98) then --达到99个，与子偕老啦
				...
				Msg2Player("你的执子之手卡已经积累了99个，你与"..otherName.."共同开启了此卡的能力。你获得了2个小时的幸运增加30点。")
				SetTask(TID_CARDNUMBER,SetByte(GetTask(TID_CARDNUMBER),GETBYTE,myNum)) --回写信息
				PlayerIndex = otherIndex
				AddSkillState(530,15,1,2*60*60*18)
				AddSkillState(533,1,1,2*60*60*18)
				Msg2Player("你的执子之手卡已经积累了99个，你与"..myName.."共同开启了此卡的能力。你获得了2个小时的幸运增加30点。")
				SetTask(TID_CARDNUMBER,SetByte(GetTask(TID_CARDNUMBER),GETBYTE,myNum)) --回写信息
			end
			PlayerIndex = oldPlyIdx
			return 0
		...
		PlayerIndex = nPlayerIndex
		Msg2Player("你得到一张执子之手卡，与"..otherName.."的缘份更近了，你现在积累了"..myNum.."张执子之手卡片。")
		SetTask(TID_CARDNUMBER,SetByte(GetTask(TID_CARDNUMBER),GETBYTE,myNum)) --回写信息
		PlayerIndex = otherIndex
		Msg2Player(myName.."得到一张执子之手卡，与你的缘份更近了，你现在积累了"..otherNum.."张执子之手卡片。")
	...
	Msg2Player("你得到一张执子之手卡，并且和"..otherName.."开始了情缘。")
	SetTask(TID_VALIDATION,code)
	PlayerIndex = otherIndex
	Msg2Player(myName.."得到一张执子之手卡，并且和你开始了情缘。")
	SetTask(TID_VALIDATION,code)
	PlayerIndex = oldPlyIdx
	return 0
end
