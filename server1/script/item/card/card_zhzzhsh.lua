...
function PickUp( nItemIndex, nPlayerIndex )
	...
	if(myCode == otherCode and myCode ~=0) then  --ÑéÖ¤ÂëÏàÍ¬
		if(myNum == otherNum) then --ÓÐÇéÈË£¬¿¨µÄÊýÁ¿Ò»Ñù¡£
			...
			PlayerIndex = otherIndex
			Msg2Player(myName.."µÃµ½Ò»ÕÅÖ´×ÓÖ®ÊÖ¿¨£¬°ÑÄãÃÇthªm gÇn nhau h¬n. HiÖn nay, b¹n ®· tÝch lòy"..otherNum.."Uyªn ¦¬ng MËt.")
		...
		SetTask(TID_CARDNUMBER,SetByte(GetTask(TID_CARDNUMBER),GETBYTE,myNum)) --»ØÐ´ÐÅÏ¢
		PlayerIndex = otherIndex
		Msg2Player(myName.."µÃµ½Ò»ÕÅÖ´×ÓÖ®ÊÖ¿¨£¬ÓëÄãduyªn phËn thªm gÇn nhau h¬n. B¹n hiÖn ®· tÝch lòy ®­îc"..otherNum.."Uyªn ¦¬ng MËt.")
	...
	PlayerIndex = otherIndex
	Msg2Player(myName.."µÃµ½Ò»ÕÅÖ´×ÓÖ®ÊÖ¿¨£¬²¢ÇÒºÍÄãb¾t ®Çu b¾t ®Çu cuéc t×nh duyªn. ")
	SetTask(TID_VALIDATION,code)
	PlayerIndex = oldPlyIdx
	return 0
end
