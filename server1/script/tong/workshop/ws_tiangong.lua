...
function CommonRepair(nItemIdx, nPrice, nItemPrice, nTongID, nWorkshopID)
	...
	-- »Æ½ð×°±¸ÐÞÀí¼Û¸ñ¹«Ê½µ÷ÕûÎª:100*(×°±¸µÈ¼¶^2)/Ã¿®iÓmÄÍ¾Ã¶È	
	if (quality == 1)then
...
function GET_DESC(nTongID, nWorkshopID, nType)
	...
	if (nWorkshopID ~= 0)then
		...
		if(nUseLevel > 0)then
			...
			elseif(aLevelRepairLevel[nUseLevel] == 2) then
				...
				szRepairKind = "<color=white>Trang bÞ tr¾ng<color>, <color=water>trang bÞ xanh<color>, <color=Violet>trang bÞ tÝm<color>¡¢<color=gold>»Æ½ð×°±¸<color>"
			end
		else
			nScale = "--"
			dAdvanceRepairCount = "--"
	...
	if (nUseLevel < wsGetMaxLevel(nTongID, nType))then
		...
		elseif(aLevelRepairLevel[nUseLevel + 1] == 2) then
			...
			szNextRepairKind = "<color=white>Trang bÞ tr¾ng<color>, <color=water>trang bÞ xanh<color>, <color=Violet>trang bÞ tÝm<color>¡¢<color=gold>»Æ½ð×°±¸<color>"
		end
	else		
		nNextMaintainFund = "--"
		nNextOpenFund = "--"
	...
	if (nWorkshopID ~= 0)then
		...
		local szMsg = "Tr¹ng th¸i t¸c ph­êng: "..state.."<color>\n"..
			...
			"<color=water>Chñng lo¹i trang bÞ cã thÓ söa ch÷a: "..szNextRepairKind.."\n"..
