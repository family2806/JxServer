...
function stunt_huangjinling1()
	...
	if (AddItem(6, 1, 1107, 1, 1, 1) > 0) then
		...
		Msg2Tong(nTongID, GetName().."»ñµÃ 1 Kü n¨ng thµnh thÞ: »ÆKimÁî")
		TONG_ApplyAddEventRecord(nTongID, GetName().."»ñµÃ 1 Kü n¨ng thµnh thÞ: »ÆKimÁî")
	end
end
...
function wsh_getwushuangshazhen()
	...
	if (nItem > 0) then
		...
		Msg2Tong(nTongID, GetName().."»ñµÃKü n¨ng thµnh thÞ:  - ÎÞË«É±Õó")
		TONG_ApplyAddEventRecord(nTongID, GetName().."»ñµÃKü n¨ng thµnh thÞ: - ÎÞË«É±Õó")
	end;
...
function deloneface(nIdx)
	...
	Msg2Player("Hñy bá1¸ö°ï»áÎÞË«É±Õó¿ò¼Ü.");
end;
-------------------------------------------------------------
--¶ÍÎä»ê-----
...
function stunt_wushenbless1()
	...
	if (nItemIdx > 0) then
		...
		SetTask(TASKID_CITY_LASTDAY, getRelativeDay(nTongID))
		SetTask(TASKID_CITYST_TAKED, GetTask(TASKID_CITYST_TAKED) + 1)
		totempole_dec("»ñµÃ <color=yellow>ÎäÉñËÄ¸£<color>, Ê¹ÓÃºó½«»ñµÃ2Ð¡Ê±Ë«±¶¹±Ï×¶È(Ö»ÓÐËÎKim»î¶¯£¬ÐÅÊ¹ÈÎÎñ£¬Ê±¼äÌôÕ½£¬ÎäÁÖÁªÈü£¬Ò°ÛÅ£¬»Ô»ÍÒ¹).")
	else
