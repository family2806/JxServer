...
function tongmap_management()
	...
	if (nTongID == 0 or TONGM_GetFigure(nTongID, GetTongMemberID()) ~= TONG_MASTER) then
		CreateTaskSay({"<dec><npc>ChØ cã bang chñ míi cã thÓ x©y dùng khu vùc cho bang", "§­îc!/tongmap_recharge_2"});
		return
	end
	local aryTalk  = {};
	if (TONG_GetTongMap(nTongID) == 0) then
		tinsert(aryTalk, "<dec><npc>ÎÞÂÛÄÄc¸i°ï»á¶¼¿ÉÒÔ½¨Éè×Ô¼ºµÄÇøÓò¡£½¨Éè×Ô¼ºµÄÇøÓòÐèÒª <color=yellow>4 Thanh ®ång ®Ønh<color>, Ê¹ÓÃÊ±¼äÎª <color=yellow>2 ÖÜ<color>. Thanh ®ång ®Ønh¿ÉÒÔÑÓ³¤°ï»áÇøÓòÊ¹ÓÃÊ±¼ä£¬Ãµ¹åThanh ®ång ®ØnhÓÐÑÓ³¤Ò»ÖÜµÄÊ¹ÓÃÊ±¼ä.");
...
--		tinsert(aryTalk, "Ç¨ÒÆ°ï»áµØÍ¼/tongmap_change_1");
	...
	tinsert(aryTalk, "Hñy bá /tongmap_recharge_2");
...
function tongmap_create_3(nMapCopyId)
	...
	if (TONG_GetTongMap(nTongID) ~= 0) then
		CreateTaskSay({"<dec><npc>Quý bang ®· cã khu vùc bang héi råi.", "§­îc!/tongmap_recharge_2"});
		return
	end
	if (CalcEquiproomItemCount(TONGMAP_PRICE_ITEM[1], TONGMAP_PRICE_ITEM[2], TONGMAP_PRICE_ITEM[3], TONGMAP_PRICE_ITEM[4]) < TONGMAP_CREATE_PRICE) then
		CreateTaskSay({"<dec><npc>½¨Éè°ï»áÇøÓòÐèÒª4 Thanh ®ång ®Ønh!", "§­îc!/tongmap_recharge_2"});
		return
	end
	-- ¿ÛÇ®
	...
	CreateTaskSay({"<dec><npc>X©y dùng thµnh c«ng khu vùc bang héi, cã hiÖu lùc ®Õn <color=yellow>"..szExpireDate.."<color>", "TiÕp tóc gia h¹n/tongmap_recharge_1", "Rêi khái/tongmap_recharge_2"});
end
...
--	end
--	if (TONG_GetTongMap(nTongID) == 0) then
--		CreateTaskSay({"<dec><npc>¹ó°ï»¹Ã»ÓÐ°ï»áµØÍ¼£¡", "ºÃµÄ/tongmap_recharge_2"});
--		return
--	end
...
function tongmap_recharge_1()
	...
	GiveItemUI("Gia h¹n khu vùc bang héi"..TONGMAP_PRICE_ITEM[5]..","..TONGMAP_RECHARGE_PRICE.."c¸i"..TONGMAP_PRICE_ITEM[5].."c¸i".."Cã thÓ gia h¹n sö dông thªm 1 tuÇn. \n"..tongmap_get_expire_date(nTongID), "Cã thêi h¹n sö dông ®Õn ", "tongmap_recharge_2");
end
function Cã thêi h¹n sö dông ®Õn (nItemCount)
	...
	CreateTaskSay({"<dec><npc>Gia h¹n thµnh c«ng thªm <color=yellow>"..nRechargeWeek.."<color> tuÇn, cã hiÖu lùc ®Õn <color=yellow>"..szExpireDate.."<color>", "TiÕp tóc gia h¹n/tongmap_recharge_1", "Rêi khái/tongmap_recharge_2"});
end
