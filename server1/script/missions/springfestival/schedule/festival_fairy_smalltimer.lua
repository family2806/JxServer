...
function festival_affiche()
    ...
    for i = 1, FE_SMALLMAPCOUNT do
        Msg2MSGroup(FE_MISSIONID, "你需要用3种道具中的一种'<color=yellow>炮仗<color>', '<color=yellow>鼠炮<color>', '<color=yellow>升天炮仗<color>' 来驱赶怪物", i);
    end;
    
    for i = FE_SMALLMAPCOUNT + 1, nCount do
        Msg2MSGroup(FE_MISSIONID, "目前你是怪兽，需要抓到<color=yellow>迷路的小孩<color> 才能领奖!", i);
    end;
end;
