...
function join_match()
	...
	for i = 1, num_vs * 2, 2 do 
		...
		WriteLog(date("%Y-%m-%d %H%M%S,")..GetAccount().."["..GetName().."], ²Î¼Ó±ÈÈü½×¶Î£¬ÄãÄ¿Ç°µÄÕ½¼¨Îª£ºth¾ng "..org_plywin.."thua"..org_plylose.."hßa"..org_plytie..", ®· tham gia råi"..nt_gettask(CP_TASKID_ROUND))
	...
	SubWorld = org_mapindex
		...
		if (getn(playeridx) > 1) then
			...
			str = "±¾ÂÖ²Î¼ÓÈËÊý²»¹»£¬²»ÄÜÑ¡Ôñ§èi thñ cña b¹n£¬±¾ÂÖÄãth¾ng ³ö£¬»ñµÃ3·Ö."
			for i = 1, getn(playeridx) do
				PlayerIndex = playeridx[i]
				enyname = GetName()
				...
				Msg2Player(str)
				DelMSPlayer(MISSIONID, PlayerIndex, 1)
				WriteLog("Ng­êi ch¬i ["..enyname.."], ÓÉÓÚÃ»ÓÐ¶ÔÊÖ£¬Ä¿Ç°Õ½¼¨Îª£ºth¾ng "..nt_gettask(CP_TASKID_WIN)..", thua"..nt_gettask(CP_TASKID_LOSE)..", hßa"..nt_gettask(CP_TASKID_TIE)..", chung cuéc"..nt_gettask(CP_TASKID_POINT))
