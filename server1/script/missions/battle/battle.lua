...
function BattleClass:InitChannel()
	...
	for k, v in CHANNEL_ID do
		 local szId = format("第 %s场 %d", CAMP_NAME[k], self.nId)
...
function BattleClass:AskEnterMap(szName, nCamp)
	...
	local szTitle = format("第 %d场宋金开始, 想要进入战场吗", self.nId)
	local tbOpt = 
	{
		{"想", self.EnterMap, {self, szName, nCamp}},
		{"重新选择", DynamicExecuteByPlayer, {nPlayerIndex, "\\script\\missions\\battle\\protocol_gs.lua", "dialog", 1}},
		{"离开队伍"},
	}
...
function BattleClass:JoinWaiting(szName, nCamp)
	...
	if (nCamp == CAMP_SONG and nCurSong >= CAMP_PLAYER_MAX_COUNT) or
		...
		Talk(1, "", format("阵营报名人数最多为 ?%d人, 请加入别的地方或参加别的阵营.", CAMP_PLAYER_MAX_COUNT))
		return 
	end
	...
	if (nCamp == CAMP_SONG and nCurSong - nCurKim >= PLAYER_COUNT_DIFFERENCE) or
		...
		Talk(1, "", format("这个军营的军队数太多了(相差不能超过 %d人), 你可以加入别的军营.", PLAYER_COUNT_DIFFERENCE))
		return 
	end
...
function BattleClass:Run()
	...
	local pDungeonType = DungeonType["宋金连server"]
...
function BattleClass:OnClose()
	...
	if nWinCamp == 0 then
		Msg2Map(self.pRule.nMapId, format("结果化"))
	else
		Msg2Map(self.pRule.nMapId, format("%s 胜利", CAMP_NAME[nWinCamp]))
	end
	...
	for k, nCamp in {CAMP_SONG, CAMP_KIM} do
		for i=1, %List.size(self.tbWaiting[nCamp]) do
			...
			if nPlayerIndex > 0 then
				CallPlayerFunction(nPlayerIndex, Msg2Player, "宋金已结束，请重新选择队伍")
			end		
		end
		for szName, pData in %Hash.pairs(self.tbPlaying[nCamp]) do
			...
			if pData and nPlayerIndex > 0 then
				if CallPlayerFunction(nPlayerIndex, self.Data.GetPLData, self.Data, szName, "TOTALPOINT") >= MIN_POINT_LIMIT then
					...
					CallPlayerFunction(nPlayerIndex, Msg2Player, format("你得到 %d 连接server贡献.", nExtPoint))
					CallPlayerFunction(nPlayerIndex, self.Data.AddTotalPoint, self.Data, szName, nExtPoint)
				end
				CallPlayerFunction(nPlayerIndex, Msg2Player, "宋金结束")
...
{
	--{目前宋人数,目前金人数,等待宋人数,等待金人数,结果宋,结果金,结果等宋,结果等金},
	{50,30,0,0,50,30,0,0},
