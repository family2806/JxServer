...
function join_tryout()
	if (nt_gettask(CP_TASKID_ROUND) >= CP_MAXROUND ) then
		Say("Sè lÇn thi ®Êu dù tuyÓn §¹i héi Vâ l©m cña b¹n ®· ®ñ!", 0)
		return
	end
	...
	DisabledUseTownP(1)	--ÏÞÖÆÆäÔÚ×¼±¸Ï¯hßa±ÈÈüÓÃµØÄÚÊ¹ÓÃ»Ø³Ç·û
...
function join_match()
	...
	for i = 1, num_vs * 2, 2 do 
		...
		WriteLog(date("%Y-%m-%d %H%M%S,")..GetAccount().."["..GetName().."], ²Î¼ÓÕ½¶·½×¶Î£¬ÄãÏÖÔÚµÄÕ½¼¨ÊÇ£ºth¾ng "..org_plywin.."thua"..org_plylose.."hßa"..org_plytie..", ®· tham gia råi"..nt_gettask(CP_TASKID_ROUND))
		Msg2Player("½øÈë±ÈÈüÇøÓòÖ®ºó£¬²»ÄÜ×ªÒÆ±³°ü×°±¸£¬µ«ÊÇÒÀ¾É¿ÉÒÔÊ¹ÓÃ±³°ühßa¹¤¾ß±íÖÐµÄÎïÆ·¡£");
		...
		WriteLog(date("%Y-%m-%d %H%M%S,")..GetAccount().."["..GetName().."], ²Î¼ÓÕ½¶·½×¶Î£¬ÄãÏÖÔÚµÄÕ½¼¨ÊÇ£ºth¾ng "..eny_plywin.."thua"..eny_plylose.."hßa"..eny_plytie..", ®· tham gia råi"..nt_gettask(CP_TASKID_ROUND))
		Msg2Player("½øÈë±ÈÈüÇøÓòÖ®ºó£¬²»ÄÜ×ªÒÆ±³°ü×°±¸£¬µ«ÊÇÒÀ¾É¿ÉÒÔÊ¹ÓÃ±³°ühßa¹¤¾ß±íÖÐµÄÎïÆ·");
		SetDeathScript("\\script\\missions\\championship\\match\\playerdeath.lua");
	end
	SubWorld = org_mapindex
		...
		if (getn(playeridx) > 1) then
			...
			str = "Õâ¾Ö±ÈÈü²Î¼ÓÕß²»×ã£¬²»ÄÜÑ¡Ôñ§èi thñ cña b¹n£¬Õâ¾ÖÄãÒÑth¾ng £¬µÃ3·Ö."
			for i = 1, getn(playeridx) do
				PlayerIndex = playeridx[i]
				enyname = GetName()
				...
				Msg2Player(str)
				WriteLog("Ng­êi ch¬i ["..enyname.."], ÓÉÓÚÃ»ÓÐ¶ÔÊÖ£¬ÏÖÔÚµÄÕ½¼¨ÊÇ£ºth¾ng "..nt_gettask(CP_TASKID_WIN)..", thua"..nt_gettask(CP_TASKID_LOSE)..", hßa"..nt_gettask(CP_TASKID_TIE)..", chung cuéc"..nt_gettask(CP_TASKID_POINT))
