...
--ÎäÁÖ´ó»á¹ÙÔ±
function main()
	--Say("ÎäÁÖ´ó»á¹ÙÔ±£ºÎäÁÖºÀ½Ü£¬ÖğÂ¹·×ÂÒÖ®ÊÀ£»ÌìÏÂÓ¢ĞÛ£¬Æë¼¯×Ï½ûÖ®áÛ¡£±¾´ÎÎäÁÖ´ó»á×÷ÎªÇøÓòÔ¤Ñ¡Èü£¬½«»áÑ¡³ö90ÃûÎäÁÖ¸ßÊÖ×÷Îª´ú±í±¾·şµÄÑ¡ÊÖ²Î¼Ó×îÖÕµÄÈ«ÇøÈ«·şÎäÁÖ´ó»á£¡È¼·ÅÄãµÄ¼¤Çé£¬À´Í¶ÈëÕâ³¡ºäºäÁÒÁÒµÄÎäÁÖ´ó»á£¡", 1, "È·ÈÏ/want_bid_option")
...
function ensure(ncount)	--È·ÈÏÅÉ·¢
	...
	for i = 1, MemCount do
		...
		cp_checkmember_title()	--È·¶¨µ±Ç°¶ÓÔ±Éí·İ
				
		if ( IsCaptain() ~= 1 and GetTask(CP_TASKID_TITLE) ~= 9 and GetLevel() >= 90) then --Èç¹û²»ÊÇ¶Ó³¤×Ô¼º,²¢ÇÒÃ»¸ø¹ı×Ê¸ñ
...
function provide(MemberName)	--ÅÉ·¢×Ê¸ñ
	...
	biwu_setdata(LEADER_MEMBER, "LeadMember", "Count", count);	--¼ÆÊı
...
function look_list_member1(count)
	local member_tab = {}
	biwu_loadfile(LEADER_MEMBER);
	MemberCount1 = tonumber(biwu_getdata(LEADER_MEMBER, "LeadMember", "Count"));	--È¡Êı£¬Èç¹û¿ªÊ¼datÎª¿Õ»á²»»áÓĞ´í£¬ĞèÒª³õÊ¼»¯°É£¿
