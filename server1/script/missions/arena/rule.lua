...
local tbNpcList = 
{
	{szName = "储物箱", nPosX = 50112, nPosY = 102848, nNpcId = 625, szScriptPath = "\\script\\battles\\openbox.lua"},
	{szName = "药商", nPosX = 50496, nPosY = 102528, nNpcId = 203, szScriptPath = "\\script\\missions\\arena\\npc\\yaodian.lua"}
...
function tbRule:ProcResult()
	...
	if pMemberA.bDeath == 1 or pMemberB.bDeath == 1 then
		if pMemberA.bDeath == pMemberB.bDeath then
			...
			Msg2Map(self.nMapId, "两边都紫阵.")			
		elseif pMemberA.bDeath == 1 then
			self:SetResult(pMemberA, pMemberB, "LOSE")
			Msg2Map(self.nMapId, format("%s 打败 %s", pMemberB.szName, pMemberA.szName))			
		else
			self:SetResult(pMemberA, pMemberB, "VICTORY")
			Msg2Map(self.nMapId, format("%s 打败 %s", pMemberA.szName, pMemberB.szName))
		end
	else
		Msg2Map(self.nMapId, format("%s 杀伤力是 %d", pMemberA.szName, pMemberB.nReceiveDamage))
		Msg2Map(self.nMapId, format("%s 杀伤力是 %d", pMemberB.szName, pMemberA.nReceiveDamage))
		
		if pMemberA.nReceiveDamage == pMemberB.nReceiveDamage then
			self:SetResult(pMemberA, pMemberB, "DRAW")
			Msg2Map(self.nMapId, "双方体力不分胜负")
		elseif pMemberA.nReceiveDamage > pMemberB.nReceiveDamage then
			self:SetResult(pMemberA, pMemberB, "LOSE")
			Msg2Map(self.nMapId, format("%s 胜利", pMemberB.szName))
		else
			self:SetResult(pMemberA, pMemberB, "VICTORY")
			Msg2Map(self.nMapId, format("%s 胜利", pMemberA.szName))
		end
	end
	self:close()
...
function tbRule:StartBattle()
	...
	Msg2Map(self.nMapId, format(" %d秒后正式开始", %READY_TIME))
...
function tbRule:OnTime()
	if self.nState == "ready" then
		...
		Msg2Map(self.nMapId, format("准备时间还有 %d秒", self.nCountDown))
		return 18
