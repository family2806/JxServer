...
function main()
	...
	if (tonumber(date("%H")) == 19) then
		...
		cbn_mapindex = SubWorldID2Idx(321)--Tr≠Íng Bπch s¨n Nam 321		--∂´±±«¯		19£∫30
	...
	if (tonumber(date("%H")) == 21) then
		...
		damahu_boss()	--ª∆Ωbossßπi M∑ HÊ £¨ÀÊª˙≥ˆœ÷‘⁄Mπc Cao QuÀt°¢Tr≠Íng Bπch s¨n Nam °¢Tr≠Íng Bπch s¨n Bæc
	end
	
	--23:00
	if (tonumber(date("%H")) == 23) then
		mg_mapindex = SubWorldID2Idx(340)--Mπc Cao QuÀt340	--Œ˜±±±±«¯	23£∫00
		...
		cbb_mapindex = SubWorldID2Idx(322)--Tr≠Íng Bπch s¨n Bæc322		--∂´±±«¯		23£∫00
...
function damahu_boss()
   ...
   elseif (damahu_mapid == 322) then
	...
	WriteLog(" ±º‰"..date("%H%M%S")..": Xu t hi÷n BOSS Hoµng Kim 'ßπi M∑ HÊ' mapid:"..damahu_mapid..", t‰a ÆÈ x:"..posx..", y:".. posy..". ßπi M∑ HÊ (ID lµ:"..DAMAHUID[rannum][1]..", thuÈc t›nh ngÚ hµnh:"..DAMAHUID[rannum][2])
