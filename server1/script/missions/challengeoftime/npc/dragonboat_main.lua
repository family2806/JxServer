...
function want_playboat()
	...
	if (startboat == 1) then
		...
		Say("²»ºÃÒâË¼£¡Khu vùc nµy"..strlevel.."HiÖn t¹i kh«ng cã nhiÖm vô khiªu chiÕn thêi gian ®Ó tham gia. B¸o danh vµo  <color=red>®óng mçi tiÕng ®ång hç<color> b¾t ®Çu, lµ".. TIME_SIGNUP .."phót, xin h·y l­u ý th«ng b¸o cña hÖ thèng.",0)
		return
	end
...
function dragon_join()
	...
	for i = 1, GetTeamSize() do 
		...
		if (havesword < 1 and blevel == 0) then
			Say("Äôs³¾: Çë¼ûÁÂ£¬²Î¼ÓNhiÖm vô Th¸ch thøc thêi gian S¬ cÊpµÄ³ÉÔ±ÒªÓÐÒ»¸ö²»ÂÛ90¼¶ÒÔÉÏ»¹ÊÇÒÔÏÂµÄÎåÐÐÉ±ÊÖïµ£¬Çë×¼±¸ºÃÖ®ºóÀ´¼ûÎÒ",0)
		...
		elseif (havesword < 1 and blevel == 1) then
			Say("Äôs³¾: Çë¼ûÁÂ£¬²Î¼ÓNhiÖm vô Th¸ch thøc thêi gian cao cÊpµÄ³ÉÔ±ÒªÓÐÒ»¸ö²»ÂÛ90¼¶ÒÔÉÏ»¹ÊÇÒÔÏÂµÄÎåÐÐÉ±ÊÖïµ£¬Çë×¼±¸ºÃÖ®ºóÀ´¼ûÎÒ",0)
		...
		if (sub >= 0) then
			...
			if ( state == 1 and GetMSPlayerCount(MISSION_MATCH, 1) == 0 and blevel == map_isadvanced[map_map[i]] ) then
				...
				for i = 1 , getn(tabplayer) do 
					...
					--¼Ç log ¸÷ÖÖÐÔÄÜ key - Modified By DinhHQ - 20120410
