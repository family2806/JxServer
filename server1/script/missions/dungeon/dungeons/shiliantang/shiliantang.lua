				...
				if (self.nFinishCount >= self.MONSTER_LIMIT) then
					...
					for i=1,getn(self.tbMember) do
						if (self.tbMember[i].bInDungeon == 1) then
							doFunByPlayer(self.tbMember[i].nIndex, Msg2Player, "´Ì¿ÍÔÝÊ±±»¸óÏÂ°ÚÆ½ÁË£¬ÏÖÔÚ¿ÉÒÔµ½ThÝ LuyÖn §­êng³¤ÀÏ´¦Áì½±.");
						end
					end
				end
				return nRetInterval;
...
function sltDungeon:OnEnterMap()
	...
	if (self.strOwner ~= strName) then
		Talk(1,"","Õß²»ÊÇ <color=red>ThÝ LuyÖn §­êng<color> ÓÉ¸óÏÂ´ò¿ª");
		return 0;
	end
...
function sltDungeon:OnDungeonOpen(tbDungeonData)
		...
		--Ìí¼Ó´«ËÍÊ¯±®
		local nnpcidx = AddNpc(309, 1, SubWorldID2Idx(self.nDungeonId), self.tbTrapPos[1], self.tbTrapPos[2],1, "ThÝ LuyÖn §­êng³¤ÀÏ");
...
function sltDungeon:CheckOwnerInDungeon()
	...
	if (tbDungeon == nil) then
		Talk(1,"","¸óÏÂ»¹Î´´ò¿ª <color=red>ThÝ LuyÖn §­êng<color>!");
		return nil;
	end
	if (nMapId ~= tbDungeon.nDungeonId) then
		Talk(1,"","¸óÏÂ²»ÔÚ <color=red>ThÝ LuyÖn §­êng<color>, »òÕß´ò¿ª color=red>ThÝ LuyÖn §­êng<color> µÄÈË²»ÊÇ¸óÏÂ£¬²»ÄÜÊ¹ÓÃ¸Ã²Ù×÷!");
		return nil;
	end
	return 1;
end
...
function sltDungeon:CallTower()
	if (self:CheckOwnerInDungeon() == 1) then
		...
		doFunByPlayer(tbMember.nIndex, Msg2Player, format("Ä¿Ç°¸óÏÂÓÐ <color=green>%d<color> »ú»áÕÙ»½cung kþ binh", tbMember.nCallTower));
...
function sltDungeon:CallBackTower()
	if (self:CheckOwnerInDungeon() == 1) then
		...
		if (bFound == 0 or nNpcIdx == 0) then
			Talk(1,"","×î½üÃ»ÓÐcung kþ binh¿ÉÒÔÕÙ»½!");
			return
		end
		
		if (tbMember == nil) then
			Talk(1,"","Äã²»ÄÜÕÙ»½cung kþ binh");
			return
		end
		...
		tbMember.nCallTower = tbMember.nCallTower + 1;
		doFunByPlayer(tbMember.nIndex, Msg2Player, format("Ä¿Ç°¸óÏÂÓÐ <color=green>%d<color> ThÝ LuyÖn §­êngÑ«ÕÂ.", tbMember.nMedal));
		doFunByPlayer(tbMember.nIndex, Msg2Player, format("Ä¿Ç°¸óÏÂÓÐ<color=green>%d<color> »ú»áÕÙ»½cung kþ binh", tbMember.nCallTower));
