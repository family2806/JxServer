...
function main()
	...
	Say(" liªn cuéc so tµi sø gi¶  vâ l©m liªn cuéc so tµi chÝnh thøc b¾t ®Çu " .. WLLS_TAB[n_ntype].name .. ", cã thÓ thÊy ta tíi mæ nh÷ng tuyÓn thñ kh¸c ®Ých tin tøc hoÆc lµ ghi danh m×nh vµ häp thµnh ®éi ®Ých tin tøc ", getn(tb_option), tb_option)
end
...
function wlls_view_me()
	...
	Describe(wlls_get_infostr(GetName()), 3,
		" söa ®æi ta c¨n b¶n tin tøc /wlls_reg_refresh", "Söa ®æi kú väng ®éi h÷u ®Ých tin tøc /wlls_edit_need('')", "·µ»Ø")
end
...
function wlls_reg_query_menu()
	...
	Say(" liªn cuéc so tµi sø gi¶ : ng­¬i cã thÓ c¨n cø bÊt ®ång lo¹i h×nh tíi mæ tuyÓn thñ , bao gåm giíi tÝnh , m«n ph¸i , cÊp bËc .", getn(tb_option), tb_option)
end
