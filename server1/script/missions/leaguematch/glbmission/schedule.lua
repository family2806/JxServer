...
function wlls_addtroop_combat( tb_vscamp , tbLGs )
	...
	Msg2MSAll(WLLS_MSID_COMBAT, "§· vµo khu vùc chuÈn bÞ, cßn <color=yellow>"..(WLLS_TIMER_FIGHT_FREQ * WLLS_TIMER_FIGHT_PREP).."<color> gi©y n÷a sÏ b¾t ®Çu")
...
function OnTimer()
	...
	if (n_timer <= 0) then	--×¼±¸½áÊø£¬¿ªÊ¼±ÈÈü
		...
		Msg2SubWorld("Vâ l©m liªn ®Êu kÕt thóc ®¨ng ký, chÝnh thøc b¾t ®Çu!")
		...
		for n_idx, nGroupIdx in tb_sub do
			...
			if (getn(tb_mstroop) < WLLS_MIN_TEAM) then
				Msg2SubWorld("Qu¸ Ýt ®éi tham gia, "..wlls_get_desc(3).." hñy bá thi ®Êu")
				tb_mstroop = {}
			end
		...
		for n_idx, nGroupIdx in tb_sub do
			...
			if (bReport) then
				SubWorld = n_idx
				Msg2MSAll(WLLS_MSID_SCHEDULE, "Thêi gian chuÈn bÞ <color=yellow>"..(n_timer*WLLS_TIMER_PREP_FREQ/60).."<color> phót")
			end
		end
