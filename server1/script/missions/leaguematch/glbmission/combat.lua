...
function OnTimer()
	...
	for n_idx, nGroupIdx in tb_sub do
		...
		elseif (n_timer < WLLS_TIMER_FIGHT_PREP) then
			...
			Msg2MSAll(WLLS_MSID_COMBAT, "Thêi gian chuÈn bÞ: Cßn l¹i <color=yellow>"..n_resttime.."<color> gi©y.")
		else
			...
			elseif (mod(n_timer, 2) == 0) then	--Ë«Êý´Î±¨¸æ£¬µ¥Êý²»±¨¸æ
				...
				for i = 1, WLLS_MAX_COUNT, 2 do
					...
					if (count_org > 0 or count_emy > 0) then --²»ÊÇË«·½¶¼Îª¿Õ
						Msg2MSGroup(WLLS_MSID_COMBAT, "<color=pink>§èi ph­¬ng cßn "..count_emy.." ng­êi", i)
						Msg2MSGroup(WLLS_MSID_COMBAT, "<color=pink>§èi ph­¬ng cßn "..count_org.." ng­êi", i + 1)
					end
				end
				--±¨¸æÉËº¦Á¿
				...
				for i = 1, getn(tb_camp) do
					...
					Msg2MSGroup(WLLS_MSID_COMBAT, "§¸nh tróng môc tiªu: <color=yellow>"..n_dmg.."<color>", tb_camp[i])
				end
				--±ÈÈüÊ£ÓàÊ±¼ä
				...
				if (n_RestMin == 0) then
					Msg2MSAll( WLLS_MSID_COMBAT, "Giai ®o¹n chiÕn ®Êu: cßn <color=yellow>"..n_RestSec.."<color> gi©y" );
				elseif (n_RestSec == 0) then
					Msg2MSAll( WLLS_MSID_COMBAT, "Giai ®o¹n chiÕn ®Êu: cßn <color=yellow>"..n_RestMin.."<color> phót" );
				end;
			end
		end
	end
