...
DaTuShaClass = Dungeon:new_type("乱战九州谷")
...
function DaTuShaClass:_init()
	...
	ClearMapNpcWithName(self.nMapId, "九州谷统领")
	...
	local tbNpc = {szName = "九州谷统领", nNpcId = 1411, nLevel = 95, szScriptPath = "\\script\\missions\\datusha\\tongshuai_npc.lua"}
	...
	for szName, tbData in self.tbPlayer do
		...
		if tbData and nPlayerIndex > 0 then
			if self.nSurvivorCount < MIN_PLAYER_COUNT then				
				CallPlayerFunction(nPlayerIndex, tbAwardTemplet.Give, tbAwardTemplet, {nExp_tl = 5e6}, 1, {"乱战九州谷","安慰奖"})
				CallPlayerFunction(nPlayerIndex, NewWorld, unpack(tbData.tbSignUpPos))
				CallPlayerFunction(nPlayerIndex, Msg2Player, "请见谅，由于报名人数太少了，这次比赛取消。")
			else
...
function DaTuShaClass:PrepareState(nTime)	
	...
	if mod(nTime, 60) == 0 and nRemainTime > 60 then
		Msg2Map(self.nMapId, format("距离活动开始还有 %d秒, 请各位高手准备.", nRemainTime))
		return 60
...
function DaTuShaClass:ReportPersonInfo()
	...
	for i=1, getn(tbData) do
		...
		if pData and nPlayerIndex > 0 then
			CallPlayerFunction(nPlayerIndex, Msg2Player,format("积分: %d", pData.nScores))
			CallPlayerFunction(nPlayerIndex, Msg2Player,format("排名: 第%d", i ))
			CallPlayerFunction(nPlayerIndex, Msg2Player,format("重生数量还有: %d", pData.nLife))
		end
...
function DaTuShaClass:Forecast(nTime)	
	...
	if nAreaId and self.tbArea[nAreaId] then
		local szMsg =format("<color=yellow>[%s]<color>区", self.tbArea[nAreaId].szName)
		szMsg = format("%s 排<color=yellow>%d<color>秒后将关闭, 请各位离开此区域!", szMsg, nTime)
...
function DaTuShaClass:AreaClose()
	...
	if self.tbArea[nAreaId] then
		...
		szMsg = format("%s 区重来", szMsg, nTime)
		Msg2Map(self.nMapId, szMsg)
	...
	if self.nSurvivorCount == 1 then
		for szName, pData in self.tbPlayer do
			...
			if pData and nPlayerIndex > 0 then	
				...
				CallPlayerFunction(nPlayerIndex, Msg2Player, "祝大侠成为最终的勇士.")
			end
		end
	end
