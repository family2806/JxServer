...
--		SetFightState(1);
	...
	local szMsg	= format("比赛开始，时间还剩<color=yellow>%d<color> 分钟.", self.nDuelTime - nTimerCount)
	Msg2Player(szMsg)
	return 1
end
...
function huashanqunzhan:OnClose()
	...
	Msg2MSAll(self.nMissionId, "胜败已定.")
...
function huashanqunzhan:OnTimer()
	...
	local nPlayerCount = GetMSPlayerCount(self.nMissionId, 0)
	local szMsg	= format("剩余时间为 <color=yellow>%d<color> 分钟.", self.nDuelTime - nTimerCount)
	...
	if nTimerCount >= self.nDuelTime or nPlayerCount <= 1 then
		...
		Msg2MSAll(self.nMissionId, format("剩余人数为: %d",nPlayerCount))
	end
...
function huashanqunzhan:Judge(tbPlayer)
	...
	local szMsg = format("你是最后胜者, 在<color=yellow>%d<color> 分钟内可以进行领奖，如果过了时间就不能领了.", self.nLatencyTime-nCurTime)
	SetMissionS(self.tbMissionS.WINER_INDEX, szWinerName)
	AddGlobalNews(format("最后胜者<color=red>%s<color>: <color=yellow>%s<color>.", self.szMatchName, szWinerName))
