...
function pTask:OnStart()
	...
	local nIndex = DlgNpcManager:AddNpc("路彪", m_BossId, task:GetMapId(), pos_boss.x, pos_boss.y, self)
	if (nNpcIndex <= 0) then
...
function pTask:Say(player)
	if (self.m_Step == 1) then
		return "请交 <color=red> 50 万<color>两", {"交", "不交", "让我想想!"}
	elseif (self.m_Step == 2) then
		return format("请给我<color=red>%s<color>.", self.m_Items[self.m_Want]), {"给 ", "不给! ", "让我想想!"}
	end
end
...
function pTask:BossChange()
	...
	DlgNpcManager:DelNpc(self.m_DlgBossIndex)
	self.m_DlgBossIndex = 0
	self.m_FightBossIndex = FightNpcManager:AddNpc("路彪", self.m_Task.m_BossId, self:GetMapId(), pos_boss.x, pos_boss.y, self, 0, 1, 1)
end
...
function pTask:OnAnswer1(player, sel)
	...
	elseif (sel == 2) then
		...
		self.m_Monsters = {}
		for i = 1, 30 do
			local nNpcIndex = FightNpcManager:AddNpc("路彪手下", self.m_Task.m_MonsterId, self:GetMapId(), pos_monster.x, pos_monster.y, self, i, 1, 1)
...
function pTask:OnAnswer2(player, sel)
	...
	elseif (sel == 1) then
		...
		if (not item) then
			player:Say(format("你没有 color=red>%s<color>.", self.m_Items[self.m_Want]))
		elseif (self:DelItem(player, item) ~= 1) then
			player:Say(format("喂, <color=red>%s<color> 你的有问题.", self.m_Items[self.m_Want]))
		else
			self:Proceed()
		end
	end
