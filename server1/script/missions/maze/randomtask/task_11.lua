-- 11.	ÓÐ¸öÃÀÀöµÄÉÙÅ®Ph¸ O¶n Nhi£¬µØÉÏÓÐºÜ¶àhoa£¬ÉÙÅ®ÒªÇóÍæ¼Ò½»ÄÉ1ÒÚÁ½Òø×Ó£¬·ñÔò²É¼¯×îºìµÄÒ»¶ähoa£¬
-- ²»ÂÛ²ÉÁËÄÄ¶ä£¬¶¼²»ÄÜÍê³ÉÈÎÎñ£¬Òª²É¼¯ºÍÉÏ½»3´Î£¬È»ºóPh¸ O¶n Nhi»¹ÊÇ²»ÂúÒâ£¬ÕÙ»½³ö11¸öÐ¡Ph¸ O¶n Nhi£¬»÷°ÜËùÓÐÐ¡Ph¸ O¶n Nhiºó»òÕßÉÏ½»1ÒÚÁ½Òø×Ó¹ý¹Ø¡£
...
function pTask:CallMonsters()
	...
	local mapid = self:GetMapId()
	for i = 1, 11 do
		local nIndex = FightNpcManager:AddNpc("Ð¡Ph¸ O¶n Nhi", 1705, mapid, pos[i].x, pos[i].y, self, i, 1, 1)
...
function pTask:BossSay(player)
	if (self.m_Step == 1) then
		return "Muèn v­ît ¶i kh«ng? VËy th× ®­a ®©y 500 v¹n l­îng tiÒn m·i lé phÝ ®i.", {"§­a", "²»§­a", "§Ó ta suy nghÜ"}
	elseif (self.m_Step < 5) then
		...
		if (self.m_Step == 2) then
			player:Say("Õâ¶ähoa²»ºì£¬ÄãÃÇÔÙÈ¥ÕªÐ©.")
...
function pTask:OnAnswer(player, sel, nIndex)
	if (self.m_Step == 1) then
		...
		elseif (sel == 2) then
			player:Say("²»§­a°¡£¿ÄÇ¾Í§­aÎÒÕªÒ»¶ä×îºìµÄ»°»ØÀ´.")
			self.m_Step = 2
		end
	end
end
...
function pTask:OnProgressbarTimeout(nId, tb, player)
	...
	if (tb.NpcIndex > 0) then
		local name = player:GetName()
		player:Say("ÄãÕªµ½Ò»¶äºìhoa")
		self:BroadCast(format("%s Õªµ½Ò»¶äºìhoa®á", name))
		DlgNpcManager:DelNpc(tb.NpcIndex)
		tb.NpcIndex = 0
