...
function pTask:OnStart()
	...
	for i = 1, 4 do
		local nIndex = FightNpcManager:AddNpc("柱子", 1703, self:GetMapId(), pos_pillar[i].x, pos_pillar[i].y, self, i, 1, 1)
	...
	for i = 1, self.m_MonsterCount do
		local nIndex = FightNpcManager:AddNpc("钰龙弟子", self.m_MonsterId, self:GetMapId(), pos_monster[i].x, pos_monster[i].y, self, i + 4, 1, 1)
		local nNpcIndex = FightNpcManager:GetNpcIndex(nIndex)
		SyncNpc(nNpcIndex)
