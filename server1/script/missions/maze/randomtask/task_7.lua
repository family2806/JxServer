-- 里面1个蛇王和30个蛇卵，每隔30秒有3个蛇卵会变成蛇，蛇的血比蛇卵多，蛇有攻击，蛇卵没攻击。把蛇王杀掉全部后过关。
...
function pTask:OnStart()
	...
	for i = 1, 30 do
		local nNpcIndex = FightNpcManager:AddNpc("蛇卵", 1700, mapid, pos_snake[i].x, pos_snake[i].y, self, i, 1, 1)
		if (nNpcIndex <= 0) then
	...
	local nBossIndex = FightNpcManager:AddNpc("蛇王", 1699, mapid, pos_boss.x, pos_boss.y, self, 0, 1, 1)
...
function pTask:OnTime()
	...
	for i = 1, getn(self.m_Snakes) do
		...
		if (tb.IsEgg == 1 and tb.NpcIndex > 0) then
			FightNpcManager:DelNpc(tb.NpcIndex)
			local nNpcIndex = FightNpcManager:AddNpc("小蛇", 1748, mapid, pos[i].x, pos[i].y, self, i, 1, 1)
			if (nNpcIndex <= 0) then
