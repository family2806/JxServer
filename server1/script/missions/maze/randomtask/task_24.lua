-- 24.	房间里出现3个分别带反弹、无形蛊、梦蝶的柱子，柱子光环给小怪共享，柱子可以被打死，血比小怪少，
-- 柱子周围是20个钰龙高手，中间是BOSS钰龙神兽，杀死所有小怪和BOSS才能过关
...
function pTask:OnStart()
	...
	self.m_BossIndex = FightNpcManager:AddNpc("玉龙神僧", 1727, mapid, pos_boss.x, pos_boss.y, self, 0, 1, 1)
	self.m_Pillars = {}
	for i = 1, 3 do
		local nNpcIndex = FightNpcManager:AddNpc("柱子", 1723 + i, mapid, pos_pillar[i].x, pos_pillar[i].y, self, i, 1, 1)
		tinsert(self.m_Pillars, nNpcIndex)
	end
	self.m_Monsters = {}
	for i = 1, 20 do
		local nNpcIndex = FightNpcManager:AddNpc("玉龙高手", 1695, mapid, pos_monster[i].x, pos_monster[i].y, self, i + 3, 1, 1)
