-- 22.	每隔10秒出现分散的5个钰龙高手，10秒后如果没杀死怪就会消失重新刷下一批，要求杀死50个怪才能过关
...
function pTask:OnHintTime()
	self:RoomBroadCast(format("已击杀 %d 玉龙高手", self.m_KillCount))
end
...
function pTask:OnGameTime()
	...
	for i = 1, 5 do
		local nNpcIndex = FightNpcManager:AddNpc("玉龙高手", 1695, mapid, pos[i].x, pos[i].y, self, i, 1, 1)
