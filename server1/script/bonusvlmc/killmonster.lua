		...
		if nTaskId == self.tbTaskList[i].nTaskId then
			...
			if NpcFunLib:CheckName(tbTask.szMonsterName, nNpcIndex) == 1 and NpcFunLib:CheckInMap(tbTask.nMapId, nNpcIndex) == 1 then
				...
				if nKillCount < tbTask.nKillCount then
					...
					local szMsg = format("大侠 %s : <color=yellow>%d/%d<color>", tbTask.szTaskName, nKillCount + 1, self.tbTaskList[i].nKillCount)
					Msg2Player(szMsg)
				end				
			end
			return
	...
	for i=1, getn(self.tbTaskList) do
		if nTaskId == self.tbTaskList[i].nTaskId then
			...
			if tbTask.nKillCount == nKillCount  then		
				...
				local szMsg = format("大侠需要%s:<color=yellow> %d/%d<color>", tbTask.szTaskName, nKillCount, self.tbTaskList[i].nKillCount)
				Msg2Player(szMsg)
				Talk(1, "", "依然没有完成就不能收奖，请加油.")
				return
			end
		end
	end
...
function VLMC:ReviewCurKillMonsterTask()
	...
	for i=1, getn(self.tbTaskList) do
		if nTaskId == self.tbTaskList[i].nTaskId then
			local tbTask = self.tbTaskList[i]
			local szTaskName =format("请<color=yellow><%s><color> 在 <%s>", tbTask.szTaskName, tbTask.szMapName)
			local szCurKill =format("大侠已%s : <color=yellow>%d/%d<color>", tbTask.szTaskName, nKillCount, self.tbTaskList[i].nKillCount)
