...
function BonusEveryDay()
	...
	if (CheckDayOfWeek() == 1) then
			...
			if (nTongID < 1) then
				Say("<#>  你未加入帮会. 将不能获得与帮会相关的奖励", 2,
				"我想领/GetBonusFirstWeek",
				"放弃/Cancel")
			else
				GetBonusFirstWeek()
			end
...
function GetBonusFirstWeek()
	...
	if (nHour >= 0 and nHour <=1400 ) then
		...
		for i=1,3 do
			...
			WriteLog(date("%Y%m%d %H%M%S").."\t".."安邦黄金令"..GetAccount().."\t"..GetName().."\t".."领取每日奖励获得"..strItem)
		end
		...
		-- Add 50 van luong
			Earn(500000)
			Msg2Player("你获得 50 万两");
			WriteLog(date("%Y%m%d %H%M%S").."\t".."安邦黄金令"..GetAccount().."\t"..GetName().."\t".."领取每日奖励获得50 万两")
	...
	if (nHour > 1400 and nHour <=2400 ) then
			...
			if (nTongID ~= 0) then
				...
				Msg2Player("你获得 2000 帮会贡献值");
				WriteLog(date("%Y%m%d %H%M%S").."\t".."安邦黄金令"..GetAccount().."\t"..GetName().."\t".."领取每日奖励获得2000 帮会贡献值")
			end
...
function GetBonusEndWeek()
		...
		if (nHour >= 0 and nHour <=1400 ) then
				...
				--Add 100 van luong
				Earn(1000000)
				Msg2Player("大侠获得 100 万两")
				WriteLog(date("%Y%m%d %H%M%S").."\t".."安邦黄金令"..GetAccount().."\t"..GetName().."\t".."领取每日奖励获得100 万两")
				--Add 3 qua huy hoang
				for i=1,3 do
				...
				-- add 1 mat na nguyen soai
					...
					-- add 5000 diem cong hien BH
						...
						if (nTongID ~= 0) then
							...
							Msg2Player("你获得 5000 帮会贡献值");
							WriteLog(date("%Y%m%d %H%M%S").."\t".."安邦黄金令"..GetAccount().."\t"..GetName().."\t".."领取每日奖励获得5000 帮会贡献值")
						end
