...
--调整兑换积分时的经验值限制- Modified by DinhHQ - 20110810
--Limit_Exp = 550000
--Limit_Exp = 700000
tbLimit_Exp = {
						[0] = 700000,
...
function main(sel)
	...
	if (state == 0 or state == 1) then
		...
		Talk(1,"","军需官：前方战争正在激烈进行，各位需要暂时躲避一下!")
		SubWorld = tempSubWorld;
		return
	end;
	SubWorld = nOldSW;	
...
function maintalk()
	...
	--弹出对话框
	tbDailog.szTitleMsg = "军需官：你需要什么？"
	tbDailog:AddOptEntry("我想买道具", songshop_sell)
	tbDailog:AddOptEntry("我想兑换经验值", exp_exchange)
	tbDailog:AddOptEntry("使用岳王魂之石来炼岳王剑", yuewang_want)
	...
	--暂时做剑冢迷宫- Modified by DinhHQ - 20110810
...
function exp_exchange()
	--Say("本功能暂不开放，敬请期待！", 0 )
	if( GetLevel() < 40 ) then
		Talk( 1, "", "军需官：你没有达到40级，不能参加战场，怎么可以用积分换取经验值");
	else
