...
function main()
	...
	if (n_level == 160) then
		...
		if (n_transcount == 3 and GetTask(TSK_ZHUANSHENG_AWARD) == 1) then		-- 当前是三转，并且在190级转生
			 		Msg2Player(format("恭喜大侠转生%d 次后，达到 %d级, 可以到 %s 领取奖励: %s",
			 					3, 160, "B綾 u L穙 Nh﹏", "Phi猲 V? "));
			 		WriteLog(format("[3rdTransLife]\t%s\tAccount:%s\tName:%s\tRank:%d",
			 					GetLocalDate("%Y-%m-%d %X"),GetAccount(), GetName(),n_count))
		end
	end
--	wlls_leveupcheck()	--武林新秀联赛越级检查
end
