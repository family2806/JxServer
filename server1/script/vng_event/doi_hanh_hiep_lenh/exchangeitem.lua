...
function Do_Recover(nCount)
	...
	for i = 1, nCount do
		...
		if IsMyItem(nIdx) ~= 1 or RemoveItemByIndex(nIdx) ~= 1 then
			...
			tbLog:PlayerActionLog("Phôc håi Hµnh HiÖp LÖnh", "XoaHanhHiepLenhCu", "SoLuong: "..nRemovedCount)
		end		
	end
	local tbAward = {szName = "Hµnh HiÖp LÖnh", tbProp = {6,1,2566,1,0,0}, nCount = nItemCount}
	tbAwardTemplet:Give(tbAward, 1, {"Phôc håi Hµnh HiÖp LÖnh", "ÁìÈ¡ÐÂHµnh HiÖp LÖnh"})
end

function tbVng_Exchange_HHL:Recover_Item_UI()
	GiveItemUI("Hµnh HiÖp LÖnh½±Àø", "Çë·ÅÈëHµnh HiÖp LÖnh", "Do_Recover", "OnCancel")
end
