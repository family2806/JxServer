...
_Message =  function (nItemIndex)
	...
	local msg = format("<color=green>Íæ¼Ò <color=yellow>%s<color=green> ÖÖC©y §a¼°»ñµÃ½±Àø<color=yellow><%s><color>" ,GetName(),GetItemName(nItemIndex))
...
function tbSeed:AddPlantCount(kind)
	...
	elseif kind == 2 then
		local tbLogTittle = {
			[50] = "Ê¹ÓÃ50´ÎC©y §aÖÖ×Ó",
			[100] = "Ê¹ÓÃ100´ÎC©y §aÖÖ×Ó",
			[200] = "Ê¹ÓÃ200´ÎC©y §aÖÖ×Ó",
			[300] = "Ê¹ÓÃ300´ÎC©y §aÖÖ×Ó",
			[400] = "Ê¹ÓÃ400´ÎC©y §aÖÖ×Ó",
			[500] = "Ê¹ÓÃ500´ÎC©y §aÖÖ×Ó",
			[600] = "Ê¹ÓÃ600´ÎC©y §aÖÖ×Ó",
			[700] = "Ê¹ÓÃ700´ÎC©y §aÖÖ×Ó",
			[800] = "Ê¹ÓÃ800´ÎC©y §aÖÖ×Ó",
			[900] = "Ê¹ÓÃ900´ÎC©y §aÖÖ×Ó",
			[1000] = "Ê¹ÓÃ1000´ÎC©y §aÖÖ×Ó",
			[1100] = "Ê¹ÓÃ1100´ÎC©y §aÖÖ×Ó",
			[1200] = "Ê¹ÓÃ1200´ÎC©y §aÖÖ×Ó",
			[1300] = "Ê¹ÓÃ1300´ÎC©y §aÖÖ×Ó",
			[1400] = "Ê¹ÓÃ1400´ÎC©y §aÖÖ×Ó",
			[1501] = "Ê¹ÓÃ1500´ÎC©y §aÖÖ×Ó",
		}
		local nUsedCount = self:GetPlantCount(kind)
		if tbLogTittle[nUsedCount] then
			tbLog:PlayerActionLog(LOG_HEAD,tbLogTittle[nUsedCount])
...
function tbSeed:Grow(Kind)
	...
	if Kind == 2 and tbVNG_BitTask_Lib:getBitTask(tbTaskInfo[Kind]) == 1501 then
		local tbAward = {szName="C©y §aÖÖ×Ó",tbProp={6,1,30158,1,0,0},nCount=1, nExpiredTime = 20111201}
		local tbTranslog = {strFolder ="201111_EventTrongCay/", nPromID = 19, nResult = 1}	
		tbAwardTemplet:Give(tbAward, 1, {"Event_TrongCay", "Ê¹ÓÃ1501´ÎC©y §aÖÖ×ÓÁìÈ¡ÉñÃØ±¦Ïä", tbTranslog})
	end
...
function tbTree:Upgrade()
	...
	if (not state) then
		DelNpc(self.Npc)		
		if self.State.nDidGetAward == 1 then --ÒÑÁì½±
