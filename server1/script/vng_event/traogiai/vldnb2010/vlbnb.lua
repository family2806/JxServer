...
function GetBonusVLDNB2010_main()
	...
	local str = "<#> 请大侠选择奖励";
			...
			if (numCLBHB>0) then
				tinsert(tbOpp,"我想领 " .. numCLBHB .. " 红包俱乐部/#VLDNB_GetBonusCLBHB("..numCLBHB..")");
			end
			if (numTinhLuc>0) then
			tinsert(tbOpp,"我想领 " .. numTinhLuc .. " 血量/#VLDNB_GetBonusTinhLuc("..numTinhLuc..")");
			end
			if (numHVL>0) then
			tinsert(tbOpp,"我想领 " .. numHVL .. " 玄猿令/#VLDNB_GetBonusHuyenVienLenh("..numHVL..")");
			end
			if (numPhienVu>0) then
			tinsert(tbOpp,"我想领 " .. numPhienVu .. " 翻羽 #VLDNB_GetBonusPhienVu("..numPhienVu..")");
			end
			if (numHCD>0) then
			tinsert(tbOpp,"我想领 " .. numHCD .. " 黄真丹/#VLDNB_GetBonusHoangChanDan("..numHCD..")");
			end
...
function VLDNB_GetBonusHoangChanDan(nCount)
	...
	Msg2Player(format("大侠刚刚领了%d 黄真丹", nCount))
	WriteLog(date("%Y%m%d %H%M%S").."\t".."Bonus VLDNB2010"..GetAccount().."\t"..GetName().."\t".."领取黄真丹奖励数量"..nCount)
end
...
function VLDNB_GetBonusPhienVu(nCount)
	...
	Msg2Player(format("大侠刚刚领了 %d 翻羽", nCount))
	WriteLog(date("%Y%m%d %H%M%S").."\t".."Bonus VLDNB2010"..GetAccount().."\t"..GetName().."\t".."Nh薾 thng Phi猲 V? s? lng "..nCount)
end
...
function VLDNB_GetBonusHuyenVienLenh(nCount)
	...
	Msg2Player(format("大侠刚刚领了 %d 玄猿令", nCount))
	WriteLog(date("%Y%m%d %H%M%S").."\t".."Bonus VLDNB2010"..GetAccount().."\t"..GetName().."\t".."领取玄猿令奖励数量"..nCount)
end
...
function VLDNB_GetBonusCLBHB(nCount)
	...
	Msg2Player(format("大侠刚刚领了 %d 红包俱乐部", nCount))
	WriteLog(date("%Y%m%d %H%M%S").."\t".."Bonus VLDNB2010"..GetAccount().."\t"..GetName().."\t".."领取红包俱乐部奖励数量"..nCount)
end
...
function VLDNB_GetBonusTinhLuc(nCount)
	...
	AddEnergy(nCount)
	Msg2Player(format("大侠刚刚领了 %d 血量", nCount))
	WriteLog(date("%Y%m%d %H%M%S").."\t".."Bonus VLDNB2010"..GetAccount().."\t"..GetName().."\t".."领取血量奖励数量"..nCount)
end
