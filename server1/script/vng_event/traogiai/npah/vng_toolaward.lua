--帮助颁奖给战胜的玩家 - Created by DinhHQ - 20110607
	...
	for i = 1, getn(tbTemp) do		
		if tbTemp[i] then
			...
			if  tbExtPointLib:GetBitValue(tbExtPoint.nExtPoint, tbExtPoint.nBit) == 1 then			
				...
				tinsert(tbDiag, format("领奖 %d/#tbVngToolAward:GetAward(%d, %d)", nOption, nSeasonID, i))
			else
				strDesc = strDesc.."\n<color=red>已领奖<color>"
			end
		end
	end
	tinsert(tbDiag, "сng/OnCancel")
