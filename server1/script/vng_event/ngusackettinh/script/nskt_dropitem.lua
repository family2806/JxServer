...
--					PlayerFunLib:GetItem({tbProp=tbNSKTList[i],nExpiredTime=tbNSKT_head.nEndDay,}, 1, "Event 五色结晶_杀手 90" )
--				end
--			end
		local tbNSKTList = tbNSKT_head:getRandomKT(12)
			...
			for i = 1, getn(tbNSKTList) do
				if(tbNSKTList[i].nCount > 0)then
					PlayerFunLib:GetItem({tbProp = {6,1, tbNSKTList[i].nID, 1, 0, 0}, nExpiredTime = tbNSKT_head.nEndDay,}, tbNSKTList[i].nCount, "Event 五色结晶_杀手 90" )
				end
			end
		return 1;	
	end	
...
--			if(getn(tbNSKTList) > 0)then
--				for i = 1, getn(tbNSKTList) do
--					tbDropTemplet:GiveAwardByList(nNpcIndex, PlayerIndex, {tbProp=tbNSKTList[i], nExpiredTime=tbNSKT_head.nEndDay,}, "在五色晶石活动中落下的水贼首领", 1)
--				end
--			end
			...
			for i = 1, getn(tbNSKTList) do
				if(tbNSKTList[i].nCount > 0)then
					tbDropTemplet:GiveAwardByList(nNpcIndex, PlayerIndex, {tbProp = {6,1, tbNSKTList[i].nID, 1, 0, 0}, nExpiredTime=tbNSKT_head.nEndDay,}, "在五色晶石活动中落下的水贼首领", tbNSKTList[i].nCount)
				end
			end
		return 1;
	end	
