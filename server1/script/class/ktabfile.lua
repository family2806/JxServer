...
--	构造函数，
--	:load(filename,key)
--	Жng nh藀名为filename的tabfile，
--	:release()
--	删除Жng nh藀的tabfile内容，并释放存储空间．
--	:getFilds()
--	返回表头字段的数组．
--	:getField(col)
--	得到表头的某个字段名
...
--最后修改日期：2005-1-29
--功能叙述：
--	实例化时自动Жng nh藀名为filename的tabfile，此filename
--是描述的路径是针对游戏路径/settings的。key为标记该
--tabfile的内存唯一标识
--参数：
--	filename：要Жng nh藀的tabfile路径
--	key：唯一标识
--返回值：
--	无
--用例：
...
--最后修改日期：2005-1-29
--功能叙述：
--	Жng nh藀名为filename的tabfile，，并用key标识其在内存
--中的关键字，一旦Жng nh藀成功，所有地方都可以通过key访问
--该tabfile。
--参数：
--	filename：要Жng nh藀的tabfile的路径名
--	key：标识关键字
--返回值：
--	nil表示执行失败．否则表示成功．
--用例：
