...
--			else																--不是采花状态
				...
				if (nparam4 > 0 and nmyTime == 0) then
					...
					local szmsg = "";
					if (pure > 0) then
						szmsg = format("<color=yellow>%d a<color> 特别红玫瑰",pure);
					end;
					if (norm > 0) then
						szmsg = format("<color=yellow>%d a<color> 一般红玫瑰 %s",norm, szmsg);
					end;
					if (bud > 0) then
						szmsg = format("<color=yellow>%d a<color> 玫瑰花开%s",bud, szmsg);
					end;
					
					szmsg = format(DEC_ROSETREE_EVENT[5], szmsg);
					Say(szmsg, 2, "采摘玫瑰花/#sure2pickrose("..nNpcIdx..")", "等下回来/OnCancel");
				elseif (nmyTime <= nCurTime) then
					...
					Msg2Player("你得到一朵花"..GetItemName(nItem));
...
function sure2pickrose(nNpcIdx)
	...
	if (nparam4 > 0) then
		...
		Msg2Player("等待3秒后才能采摘玫瑰");
	else
		Talk(1, "", DEC_ROSETREE_OTHER[random(getn(DEC_ROSETREE_OTHER))]);
		Msg2Player("所以玫瑰已经采摘完");
	end;
end;
...
function sure2rosetaketask(nNpcIdx)
	...
	if (task >= 1 and task <= 4 and nstate == 0) then
		...
		Msg2Player(format("等 %s秒后才能%s.", TB_ROSETREE_TASKTIME[task], DEC_ROSETREE_STASK[task]));
	end;
end;
...
function sure2rosedotask(nNpcIdx)
	...
	if (task >= 1 and task <= 4 and nstate == 1) then
		...
		if ((GetSex() == nsex and task >= 1 and task <= 2) or (task>=3 and task <= 4)) then
			...
			Msg2Player("多谢"..DEC_ROSETREE_STASK[task]);
		elseif (GetSex() ~= nsex) then
			local szmsg = format("只有%s m才能?%s.", DEC_ROSETREE_SZSEX[nsex], DEC_ROSETREE_STASK[task]);
			Say(szmsg, 0);
		end;
	end;
end;
...
function calcrosetreepoint(nNpcIdx, nvalue)
	...
	Msg2Team("此次点数"..npoint);
	Msg2Team("总分数获得"..(ncurpoint+npoint));
