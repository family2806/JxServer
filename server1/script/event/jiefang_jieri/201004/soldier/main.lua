...
function Soldier2010:SongjinSoldier(nPlayerIdx)
	...
	if (PlayerFunLib:CheckBT_PL_BATTLEPOINT(3000, ">=") == 1) then
		tbAwardTemplet:GiveAwardByList(FreedomEvent2010.tbSongjinSoldier, format("%s\t%s","战士服活动","宋金积累点在3000以上有奖励"));
	end
	PlayerIndex = nOldIdx;
end
...
function Soldier2010:MonsterDrop(nNpcIndex, nPlayerIndex)
	if (NpcFunLib:CheckNormalMonster("10,20,30,40,50,60,70,80,90", nNpcIndex) == 1) then
		for i=1,getn(FreedomEvent2010.tbMonsterDrop) do
			tbDropTemplet:GiveAwardByList(nNpcIndex, nPlayerIndex, FreedomEvent2010.tbMonsterDrop[i], format("%s\t%s","战士服活动","普通散怪"), 1);
		end
	end
end
...
function Soldier2010:UseSoldierBag()
	if (PlayerFunLib:CheckFreeBagCell(1,"default") == 1) then
		tbAwardTemplet:GiveAwardByList(FreedomEvent2010.tbSoldierToolBag, format("战士服活动\t使用 %s","工具包"));
		return 1;
	end
	return 0;
end
...
function Soldier2010:UseSoldierStick()
	...
	if (nCurExp >= (nTransLife+1)*self.SOLDIER_STICK_EXP_LIMIT) then
		Talk(1, "", format("通过使用 [%s] 最多只有 %s 经验","常山棍",format("<color=green>%0.f<color=green>",(nTransLife+1)*self.SOLDIER_STICK_EXP_LIMIT*100000)));
		return 0;
	end
	tbSoldier_TaskGroup:AddStickExp(500);
	PlayerFunLib:AddExp(50000000,0,format("战士服活动\t使用 %s","常山棍"));
	return 1;
end
...
function Soldier2010:UseSoldierCot()
	...
	if (nCurExp >= self.SOLDIER_COT_EXP_LIMIT) then
		Talk(1, "", format("通过使用 [%s] 最多只能得到 %s 经验","常山网",format("<color=green>%d<color=green>",self.SOLDIER_COT_EXP_LIMIT*100000)));
		return 0;
	end
	tbSoldier_TaskGroup:AddCotExp(150);
	PlayerFunLib:AddExp(15000000,0,format("战士服活动\t使用 %s","常山网"));
	return 1;
end
...
function Soldier2010:UseSoldierHat()
	...
	if (nCurExp >= self.SOLDIER_HAT_EXP_LIMIT) then
		Talk(1, "", format("通过使用 [%s] 最多只能得到 %s 经验","战士帽",format("<color=green>%d<color=green>",self.SOLDIER_HAT_EXP_LIMIT*100000)));
		return 0;
	end
	tbSoldier_TaskGroup:AddHatExp(8);
	PlayerFunLib:AddExp(800000,0,format("战士服活动\t使用 %s","战士帽"));
	return 1;
end
...
function Soldier2010:UseSoldierShoes()
	...
	if (nCurExp >= self.SOLDIER_SHOES_CLOTH_EXP_LIMIT) then
		Talk(1, "", format("通过使用 [%s] 最多只能得到 %s 经验",format("%s 和 %s","部队鞋","战士服"),format("<color=green>%0.f<color=green>",self.SOLDIER_SHOES_CLOTH_EXP_LIMIT*100000)));
		return 0;
	end
	tbSoldier_TaskGroup:AddShoesClothExp(20);
	PlayerFunLib:AddExp(2000000,0,format("战士服活动\t使用 %s","部队鞋"));
	return 1;
end
...
function Soldier2010:UseSoldierCloth()
	...
	if (nExp > 0) then	
		...
		PlayerFunLib:AddExp(nExp*100000,0,format("战士服活动\t使用 %s","战士服"));
	end
	...
	tbAwardTemplet:GiveAwardByList(tbItem, format("战士服活动\t使用次 %d %s", nClothCount, "战士服"));
	return 1;
end
