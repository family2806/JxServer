  ...
  local tbRoomItems =  self:GetRoomItems(self.nRoomType) --  Ã¿´Î¶¼ÖØÐÂ»ñÈ¡£¬ÒÔ·ÀÖ¹ÓÐ±ä»¯	
	...
	for i = 1, getn(tbRoomItems) do 
		...
		if nItemQuality == 1 then
			...
			if nEqIndex >= 2055 and nEqIndex <= 2284 then -- Trang bÞ Kim ¤
				--¼ì²é°ó¶¨×´Ì¬
...
function refine_jinwu()
	...
	local strDesc = "<npc>ÄÃ <color=yellow>Trang bÞ Kim ¤<color> ÒªÖØÁ·µÄºÍÊýÁ¿<color=yellow> Kim ¤ Trïng LuyÖn Ngäc<color> ÏàÓ¦µÄ·ÅÈëÏÂÃæ"
	strDesc = strDesc..format("\n\t\t\t\t\t%-29s%s", "Trang bÞ Kim ¤", "Kim ¤ Trïng LuyÖn Ngäc")
	strDesc = strDesc..format("\n\t\t\t<color=yellow>%-34s<color=red>%d<color>", "Kim ¤ Kh«i", 2)
	strDesc = strDesc..format("\n\t\t\t<color=yellow>%-34s<color=red>%d<color>", "Kim ¤ Y Phôc·þ", 3)
	...
	tinsert(tbOpt, {"ÖØÁ·Trang bÞ Kim ¤", p.ComposeGiveUI, {p}})
