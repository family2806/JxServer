...
--	end
	...
	for i = 1, nItemCount do
		if (GetGiveItemUnit(i) ~= nEquipItemIdx and RemoveItemByIndex(GetGiveItemUnit(i)) ~= 1) then
			WriteLog("[Task Th¨ng cÊp trang bŞ B¹ch Kim ERROR!!]"..GetLocalDate("%Y-%m-%d %H:%M:%S").."\t"..GetAccount().."\t"..GetName().."\t Xãa vËt phÈm thÊt b¹i\t"..GetItemName(GetGiveItemUnit(i)));
			return
		end
	end
	if (random() > nUpGradeProb) then
		...
		WriteLog("[Task Th¨ng cÊp trang bŞ B¹ch Kim FAIL!!]"..GetLocalDate("%Y-%m-%d %H:%M:%S").."\t"..GetAccount().."\t"..GetName().."\t Gi¸ trŞ l­îng kh«ng ®ñ, x¸c suÊt lµ"..nUpGradeProb);
		return
	end;
	...
	if nSuccess > 0 then
		...
		CreateTaskSay({"<dec><npc>Th¨ng cÊp trang bŞ B¹ch Kim³É¹¦£¬µÃµ½1Ì×°×½ğ×°±¸<color=yellow>"..TB_PLATINAEQ_GOLDEQ[nComposeEntryIdx][4].."<color>, cÊp"..nItemLevel..".", "Ta sÏ quay l¹i sau/no"});
		Msg2Player("Th¨ng cÊp trang bŞ B¹ch Kim³É¹¦,µÈ¼¶ÊÇ"..nItemLevel.."trang bŞ B¹ch Kim:"..TB_PLATINAEQ_GOLDEQ[nComposeEntryIdx][4]);
		WriteLog("[Task Th¨ng cÊp trang bŞ B¹ch Kim SUCCESS!!]"..GetLocalDate("%Y-%m-%d %H:%M:%S").."\t"..GetAccount().."\t"..GetName().."\t nhËn ®­îc trang bŞ B¹ch Kim"..TB_PLATINAEQ_GOLDEQ[nComposeEntryIdx][4].."\t"..szLog);
		...
		WriteLog("[Task Th¨ng cÊp trang bŞ B¹ch Kim ERROR!!]"..GetLocalDate("%Y-%m-%d %H:%M:%S").."\t"..GetAccount().."\t"..GetName().."\t Th¨ng cÊp trang bŞ B¹ch KimÊ§°Ü\t"..szLog);
		writeRecoinLog("[Th¨ng cÊp trang bŞ B¹ch KimÔ­ÁÏ]", szBeforeItemInfo);
	end;
end;
----½«»Æ½ğ×°±¸´òÔì³É°×½ğ×°±¸ END
