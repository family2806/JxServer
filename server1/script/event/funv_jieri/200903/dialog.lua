...
local _ReceiveNpcGife = function (nPoint, szLogTitle)
	...
	if nPoint <= 0 then
		Talk(1, "", "´óÏÀµÄÎïÆ·ÎÞ·¨§æi§iÓm tÝch lòy")
		return
	end
	tbFunv0903.tbTask:AddScore(nPoint)
	local szMsg = format("»ñµÃ %d §iÓm tÝch lòy", nPoint)
	Msg2Player(szMsg)
...
local tbScoreShop = 
{
	...
	{
		szComposeTitle =  "§æi",
		szFailMsg = "´óÏÀµÄ§iÓm tÝch lòy»òÒøÁ½²»×ã£¬ÎÞ·¨§æi´ËÎïÆ·.",
...
function tbFunv0903:DialogMain(nItemIndex, szDescLink)
	...
	local tbSay = 
	{
		...
		format("´óÏÀÓÐ¶àÉÙ§iÓm tÝch lòyÁË?/#%s:CheckMyScore()", self.szClassName),
		format("´óÏÀÒª§æiÒ»Ð©/#%s:BuyItemByScore(%d, [[%s]])", self.szClassName, nItemIndex, szDescLink),
		"KÕt thóc ®èi tho¹i/OnCancel"
	}
	if nItemIndex == nil or nItemIndex == -1 then
		tinsert(tbSay, 3, format("´óÏÀÏë§æi³ÉÆ· [§iÓm tÝch lòy]/#%s:GiveNpcGift(%d)", self.szClassName, nItemIndex))
	end
...
function tbFunv0903:CheckMyScore()
	...
	if bRet ~= 1 then
		Talk(2, "", szFailMsg, "ÄãµÄ§iÓm tÝch lòy²»¹»")
	else
		Talk(1, "", format("ÄãµÄ§iÓm tÝch lòyÎª: %d", nScore))	
	end
...
function tbFunv0903:BuyItemByScore(nItemIndex, szDescLink)
	...
	nItemIndex = nItemIndex or -1
	szDescLink = szDescLink or "<npc>"
	local tbSay = self.tbScoreShop:MakeOptByProductName("ÎÒÒª§æi", nItemIndex, szDescLink)
