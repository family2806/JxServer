...
function sale_heroling_xiulibao(nCount, nPrice)
	for i =1, nCount do
		...
		Msg2Player("NhËn ®­îc "..nCount.." ĞŞÀí°ü, Ê£Óàgiê ¼äÊÇ7Ngµy")
end;
-----**ĞŞÀí°ü end
...
function sale_heroling_zijingao(nCount, nPrice)
		...
		Msg2Player("NhËn ®­îc 1c¸i×Ï½ğ¸ä£¬Ê£Óàgiê ¼äÊÇ"..nRestH.." giê, "..nRestM.."phót")
end;
-----**×Ï½ğ¸ä end
...
function saleby_heroling_enter(nSel)
	...
	local arySalebyHeroling = {
						...
						{"<dec>ÕâÊÇÒ»c¸iÓĞÄÏ·½É«²ÊµÄ¹â»·. »ñÈ¡Ïã½¶¹â»·ĞèÒª¹±Ï×µã <color=yellow>30 ®iÓm<color>. ¹±Ï×Öµ, Ê¹ÓÃÆÚÏŞÊÇ1ÖÜ.", "NhËn 1 vßng danh hiÖu H­¬ng Tiªu/#saleby_heroling_confirm(30,'sale_heroling_xiangjiao')", "Trë vÒ/enter_heroling", "Rêi khái/tong_cancel"},
						{"<dec>ÄãÏ²»¶·ÉĞĞÂğ? Õâc¸i¹â»·¸øÄã´øÀ´Ó¥»÷³¤¿ÕµÄ¸Ğ¾õ? ½Ó×Å£¬Ê¹ÓÃÆÚÏŞÊÇ1ÖÜ.<enter>ÏëÒªNhËn ®­îc Õâc¸iÃûºÅ¹â»·ÒªÓÃµÄ¹±Ï×µãÊÇ: <color=yellow>30 µã<color>", "NhËn 1 vßng danh hiÖu Th­¬ng ¦ng/#saleby_heroling_confirm(30,'sale_heroling_cangying')", "Trë vÒ/enter_heroling", "Rêi khái/tong_cancel"},
...
function sure_contribution_heroling(ncount)
	...
	Msg2Player("ÄãNhËn ®­îc "..ncount.." c¸i Phong Háa Anh Hïng LÖnh.");
end;
...
function sure_heroling_bosscharm()
	...
	if (nCount < 1) then
		CreateTaskSay({"<dec>°ï»á¿ÉÒÔÓÃ·é»ğÓ¢ĞÛÁî¶Ò»»1c¸iBossÕÙ»½·û. ·é»ğÓ¢ĞÛÁîµÄ»ıÀÛÃ¿c¸iÈËÒª¸ü¼ÓÅ¬Á¦.", "KÕt thóc ®èi tho¹i/tong_cancel"});
		return 0;
	end;
	...
	Msg2Player("ÄãNhËn ®­îc  "..nCount.." LÖnh bµi gäi Boss.");
end;
-----¶Ò»»BossÕÙ»½·û end
...
function change_letter_honour()
	CreateTaskSay({"<dec>µ±·é»ğÓ¢ĞÛÁîÓĞ<color=yellow>120c¸i<color> ÔòÃ¿ÖÜ5£¬ÖÜ6¿ÉÒÔ²Î¼Ó»î¶¯<color=yellow>×·×ÙÃÜ¼şcolor>. ÖÜ5£¬ÖÜ6ÔÚĞÂÊÖ´åµÄÍâÃæ³öÏÖ<color=yellow>µ¾²İÈË<color> ´ø×ÅÃÜ¼ş. ËùÓĞÈËÒªÕÒµ½ÕâĞ©µ¾²İÈËÉíÉÏµÄÃÜ¼ş. µ¾²İÈË³öÏÖµÄgiê ¼äÊÇ7h30, 7h40, 7h50, ¿ÉÒÔÑ°ÕÒÃÜĞÅµÄgiê ¼äÊÇ <color=yellow>7h35, 7h45, 7h55<color>. ÄãÈ¡µÃÃÜ¼şµÄgiê ¼äÊÇ1phótÖÓ£¬Ö®ºóÒªÇ××ÔÉÏ½»µ½³¯Í¢. ³¯Í¢½«¸øÓèÄãÉÍ´Í<color=yellow>25¿éºÚÌúÈÙÓşÁîÅÆ<color>. µ¾²İÈËµÄ×ø±êÊÇ£º<enter>"
...
function sure_letter_honour()
	...
	elseif (CalcFreeItemCellCount() < 25) then
		...
		Msg2Player("NhËn ®­îc 25c¸iºÚÌúÈÙÓşÁîÅÆ");
	end;
end;
