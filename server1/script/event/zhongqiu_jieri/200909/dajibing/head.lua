...
function tbZhongqiuDajibin:AddCount()
	...
	Msg2Player(format("大侠已交 %d, 整个服务器已交%d.", nPlayerCount, nServerCount+1))
end
...
function tbZhongqiuDajibin:CheckLastAward()
	...
	--Msg2Player(format("<color=yellow>得到排名奖励%u经验.<color>", tbExp.nExp))
	local tbExp2 = {nExp_tl = nPlayerCount * 8000000}
	
	tbAwardTemplet:GiveAwardByList(tbExp2, "MidAutumn,RankAward"..nRank, 1)
	--Msg2Player(format("<color=yellow>你已经交纳%u个三色之宝并获得奖励%u经验.<color>", nPlayerCount, tbExp2.nExp_tl))
...
function tbZhongqiuDajibin:GetLianziCookie()
	...
	local tbItem = {szName = "莲藕饼", tbProp={6,1,2159,1,0,0},nCount = nItemCount, nExpiredTime=20091123}
