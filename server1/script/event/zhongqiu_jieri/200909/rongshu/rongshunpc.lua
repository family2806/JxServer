...
function main()
	...
	if nCount <= 0 then
		return Talk(1, "", "此树已落完叶子了，请等发芽时再继续采摘")
	end
	
	if CalcFreeItemCellCount() < 2 then
		return Talk(1, "", format("装备需要剩余 d?%d ?", 2))
	end
	...
	if nLock == 0 then
		...
		if tbZhongQiu200909_rongshu[PlayerIndex].nTime + 15 <= GetCurServerTime() then --已经等待15秒
			--给叶子
			local tbLeaf = 
			{
				[1496] = 	
			...
			Msg2Player(format("%d ，秒后才能采摘.", tbZhongQiu200909_rongshu[PlayerIndex].nTime + 15 - GetCurServerTime()))
		end
