...
--当who == 9999时，表示门派赛，此时不存放自已和对手信息，因为每项比赛的选手在同一地图比赛
	if (who == 9999) then
		if (matchtype > 0 and matchtype <= 10) then
			...
			print("门派塞: type"..matchtype.."区域为"..map)
		end
		return
	end
	...
	if (matchtype >= 1 and matchtype <= 10) then
		for i = 1, 32 do 
			if (GetGlbValue(GLB_MATCHLEAGUEINFO + 20 + (matchtype - 1)  * 32 * 2 + i * 2 ) == 0) then
				...
				print("门派淘汰赛: type"..matchtype.." "..who.." VS "..oppt)
				return
			end
		end
	else 
--非门派赛时，GLB_MATCHLEAGUEINFO -> GLB_MATCHLEAGUEINFO + N * 2存放比赛的信息 第n个存放战队号n的所在报名地图以及对手的战队编号
		...
		print("比赛"..matchtype.." map:"..map.." "..who.." VS "..oppt)
	end
end
...
function wulin_openzonematch(signmapid, matchphase, zoneid1, zoneid2)
	...
	local resultstr = "现在，武林大会区域团体赛已经开始! 开战双方为：["..tongname1.."] 和["..tongname2.."], 请两区域的选手抓紧到城区参赛!";
...
Msg2Player("坐稳了吗？咱们走"..GetStationName(nStation))
...
function ShowSeed(worldidx, mapid, seedlevel, count, SeedPosFile,szMapName, nBatch)
	...
	local nBeginNumber = gb_GetTask("辉煌种子",12)
	local nNpcTmpl = 1110
	...
	if mod(nBatch	,2 ) == 0 then
		...
		ClearMapNpcWithName(mapid, "黄金种子");
		if seedlevel == 4  then
			nNpcTmpl = 1118
	...
	for i = 1, count do
			...
			if (nil ~= nPosX and nil ~= nPosY) then
				...
				if (nNPCIndex > 0) then
					...
					gb_SetTask("辉煌种子",12,nBeginNumber)
					...
					if szNpcName == "黄金之果实" or szNpcName == "黄金种子" then
...
function festival_shrewmouse(nMapID)
	...
	local nState = gb_GetModule("2006春活动") 
    if (nState == 1) then
