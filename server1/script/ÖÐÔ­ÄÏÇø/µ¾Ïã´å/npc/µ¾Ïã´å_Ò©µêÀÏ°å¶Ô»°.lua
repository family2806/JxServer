...
function main()
	...
	if (UTask_world38 == 1) then		--ÒÑ½ÓÈÎÎñ
  		Say("¤ng néi TiÓu Lan lµ kh¸ch quen, «ng ta cÇn thuèc X¹ H­¬ng Hæ Cèt Cao, 750 l­îng 1 lä, nÕu cã 2 lo¹i nguyªn liÖu X¹ H­¬ng vµ Hæ cèt, ta sÏ bèc thuèc miÔn phİ cho.", 4, "Mua trùc tiÕp/buy", "T×m nguyªn liÖu/source", "Mua lo¹i thuèc kh¸c/yes","§Ó ta suy nghÜ l¹i!/no")
	elseif (UTask_world38==2) then
		if (HaveItem(185) == 1) and (HaveItem(142) == 1) then
			...
			SetTimer(0.5 * CTime * FramePerSec, 6)									--¿ªÆô¼ÆÊ±Æ÷£º°ë¸öÊ±³½ * Ê±³½³¤ * Ö¡Êı
			...
			Msg2Player("Sau khi t×m ®ñ nguyªn liÖu ph¶i ®îi nöa tiÕng sau míi chÕ ®­îc thuèc.")
  			...
  			Say("NÕu ng­¬i cã thÓ t×m ®­îc 2 nguyªn liÖu X¹ h­¬ng vµ Hæ cèt, ta sÏ bèc thuèc miÔn phİ cho ng­¬i.", 3, "mua trùc tiÕp X¹ H­¬ng Hæ Cèt Cao!/buy","Giao dŞch/yes", "Kh«ng giao dŞch/no")
		end
	elseif (UTask_world38 == 3) then				-- Îª¼ÆÊ±Æ÷Èİ´í¼ÓÈëÊÖ¶¯³¬Ê±´¦Àí
		i = GetRestSec(6)
		if (i > 0) then
			Say("<#> Thêi gian ch­a ®ñ, thuèc ch­a lµm xong ®­îc, ng­¬i ph¶i chê ®îi thªm."..i.."<#> gi©y.", 3, "Ta kh«ng muèn ®îi n÷a, ®Ó mua trùc tiÕp cho råi!/buy","ÎÒÏëGiao dŞch/yes", "VËy th«i ®i/no")
		else
			StopTimer()						--½áÊø¼ÆÊ±
			W66_getitem()
		end
...
function buy()
  	if (GetCash() < 750) then
		...
		if (GetTimerId() == 6) then			-- ÕıÔÚºÏÒ©¼ÆÊ±ÖĞ
			StopTimer()
		end
  	end
end
