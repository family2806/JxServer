...
function pActivity:CheckComposeRedEvelopLimit(nCount)
	...
	if (nTaskVal + nCount) > 2 then
		Msg2Player(format("数量超过限制，只能换<color=yellow>%d <color>红包了.", 2 - nTaskVal))
		return nil
	end
...
function pActivity:ComposeRedEvelop(strLog, nCount)
	...
	local tbAward = {szName="宝箱",tbProp={6,1,30169,1,0,0}, nCount = nCurCount*5,nExpiredTime=20120127, nBindState = -2,}
...
function pActivity:CheckComposeDragonMask(nCount)
	...
	if (nTaskVal + nCount) > 5 then
		Msg2Player(format("数量超过界限，只能换<color=yellow>%d <color>次了.", 5 - nTaskVal))
		return nil
	end
	return 1
end
...
function pActivity:ComposeDragonMask(nCount)
	...
	for i = 1, nCurCount do
		tbVngTransLog:Write("201201_event_tet/", 22, "DoiMatNaRongNhanKinhNghiem", "10000000 经验", 1)
	end
end