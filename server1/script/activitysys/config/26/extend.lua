...
function pActivity:DrinkOneRound(TSK_A, TSK_B, TSK_Time, TSK_Round, TSK_AwardB, TSK_AwardC, TSK_Date, nSkillId, szKind, nDetailId)
	...
	self:AddTask(TSK_B, nAddValueB)--Tˆu L≠Óng
	...
	if nValueA >= 100 then
		...
		self:SetTask(TSK_B, 0)--Tˆu L≠Óng«Â¡„
...
function pActivity:Lose(TSK_A, TSK_B, TSK_C, TSK_D, TSK_E)
	...
	self:SetTask(TSK_E, 1)--Tˆu L≠Óng«Â¡„
end
...
function pActivity:ReceiveFixCountData(nParam, ResultHandle)
	...
	local nResult = DynamicExecuteByPlayer(nPlayerIndex, "\\script\\activitysys\\config\\26\\extend.lua", "pActivity:CheckTaskB", %TSK_LastTimeDate, "…Ÿœ¿“—æ≠¡Ï¡À≈≈√˚Ω±¿¯¡À", "~=")
	if  nResult ~= 1 then
		return
	end
...
function pActivity:ReceivePlayerPaiMing(nParam, ResultHandle)
	...
	if nPaiMing == nil then
		CallPlayerFunction(nPlayerIndex, Talk, 1, "", "…Ÿœ¿ƒø«∞√ª”–◊Ú»’Tˆu L≠Óng≈≈√˚")
		return
	end
