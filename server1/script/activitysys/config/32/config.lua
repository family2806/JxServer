...
tbConfig[1] = --Ò»¸öÏ¸½Ú
{
	...
	nStartDate = nil,
	nEndDate  = nil,
	tbMessageParam = {"Nam Cung NhÊt Thiªn"},
	tbCondition = 
	{
...
tbConfig[2] = --Ò»¸öÏ¸½Ú
{
	...
	nStartDate = nil,
	nEndDate  = nil,
	tbMessageParam = {"'HiÖp Chi §¹i Gi¶, V× N­íc V× D©n'. LÇn nµy ta phông mÖnh s­ phô, ®Õn ®©y ®Ó t×m c¸c ch­ vŞ anh hïng hµo kiÖt. S­ phô ta nãi r»ng chØ cÇn chóng ta ®ång t©m hiÖp lùc, ¾t sÏ cã 1 ngµy sÏ ®¸nh ®uæi ®­îc Th¸t Tö. Noµ ng­¬i cã ph¶i lµ ng­êi mµ ta muèn t×m kh«ng?",0},
	tbCondition = 
	{
		{"ThisActivity:IsOpen",	{nil} },
	},
...
tbConfig[11] = --Ò»¸öÏ¸½Ú
{
	...
	tbCondition = 
	{
		{"PlayerFunLib:CheckTask",	{TSK_TALK_TASK_ID,0,"ÕâÎ»<lua GetSex() == 1 and \"½ã½ã\" or \"¸ç¸ç\"/> ÎÒ»¹²»ĞèÒªÄã°ïÎÒËÍĞÅ",">"} },
		{"PlayerFunLib:CheckTask",	{TSK_TALK_TASK_STATE,0,"ÕâÎ»<lua GetSex() == 1 and \"½ã½ã\" or \"¸ç¸ç\"/> ÄãÒÑ¾­°ïÎÒËÍĞÅµ½´ïÁË£¬ÕæÊÇ¸ĞĞ»","=="} },
		{"PlayerFunLib:CheckItemInPlayer",	{{tbProp={6,1,2571,1,-1,0},},1,"ÕâÎ»<lua GetSex() == 1 and \"½ã½ã\" or \"¸ç¸ç\"/> ÔÚÄãÉíÉÏ»òÕßÔÚÄãµÄ´¢ÎïÏäÀïÒÑ¾­ÓĞÎÒµÄĞÅÁË£¬ÄãÔÙ×ĞÏ¸ÕÒÕÒ." } },
