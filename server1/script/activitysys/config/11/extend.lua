...
function pActivity:UseNewUniform()
	...
	tbAwardTemplet:Give(%tbNewUniformAward, 1, {%EVENT_LOG_TITLE, "使用新衣领"})
...
function pActivity:NewUniformExtraAward()
	...
	if %tbNewUniformExtraAward[nUsedCount] then
		local szAction = format("使用新衣领%dLan", nUsedCount)
		local szMsg = format("你已使用 %s%d 次, 收到超额奖励.",%tbITEM_NEW_UNIFORM.szName, nUsedCount)
		Msg2Player(szMsg)
...
function pActivity:UsePatchedUniform()
	...
	if %tbPatchedUniformLog[nUsedCount] then
		local szAction = format("使用%d 次 已改衣领", nUsedCount)
...
function pActivity:UseJade()
	...
	local szMsg = format("你已使用%s 增加限制 %d 次.", %tbITEM_NEW_UNIFORM.szName, %nJADE_INCREASE_MAX_COUNT)
	Msg2Player(szMsg)
...
function pActivity:InitFormula()
	...
	local tbGold = 
	{
		[1] = {szName="银两", nJxb=30000, nCount=1},
		[2] = {szName="银两", nJxb=60000, nCount=1},
		[3] = {szName="银两", nJxb=100000, nCount=1},
	}
	...
	for i=1, getn(tbGold) do
		for j=1, getn(tbMaterial) do
			...
			local pCompose = tbActivityCompose:new(tbFormula, "换绿色布料", INVENTORY_ROOM.room_equipment)
...
function pActivity:SelectMaterialDlg(nGoldSelect)
	local szTitle = format("请选择换<color=green>%s<color> 材料", %tbITEM_GREEN_CLOTH.szName)
	local tbOpt = 
	{
...
function pActivity:SelectGoldDlg()
	local szTitle = format("请选择方法<color=green>%s<color> 换:\n", %tbITEM_GREEN_CLOTH.szName)
	local tbText = {}
	local szGold = "%d 万两"
	local szOptText = "格式%d\t"
	for i=1,3 do
		if i < 3 then
			tbText[i] = format("每日最多换%d 次", %nMAX_COMPOSE_COUNT[i])
		else
			tbText[i] = "不限制换的次数"
		end
		tbText[i] = format(szGold.."(%s)",%tbGOLD_COUNT[i]/10000, tbText[i])
		
		szTitle = format("%s 格式d\t 材料+%s;\n", szTitle, i, tbText[i])
...
function pActivity:Exchange(nGoldSelect, nMaterialSelect)
	...
	if nMaxCount <= 0 then
		local szText = format("你通过换的形式<color=green>%s<color> 数量已达限制数，请试试其他方法.", %tbITEM_GREEN_CLOTH.szName)
		Say(szText)
		return
	end
...
function pActivity:UseNewUniformExp()	
	self:AddTask(%nIdx_USED_NEW_UNIFORM_COUNT,1)	
	tbAwardTemplet:Give(%tbNewUniformAwardExp, 1, {%EVENT_LOG_TITLE, "使用新衣领(Exp)"})	
end
...
function pActivity:NewUniformExpExtraAward()	
	...
	if %tbNewUniformExtraAward[nUsedCount] then
		...
		local szMsg = format("你已经使用%s%d次，收到超额的奖励.",%tbITEM_NEW_UNIFORM_EXP.szName, nUsedCount)
		Msg2Player(szMsg)
		tbAwardTemplet:Give(%tbNewUniformExtraAward[nUsedCount], 1, {%EVENT_LOG_TITLE, szAction})
	end	
end