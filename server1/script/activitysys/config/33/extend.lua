...
function pActivity:UseFirworks()
	...
	elseif (self:CheckTask(4, 1000,"","==") == 1) then
		...
		local tbTranslog = {strFolder = %strQK_TranslogFolder, nPromID = %nQK_PromotionID, nResult = 1}
		tbAwardTemplet:Give(tb1000Award, 1, {"Event_QuocKhanhVN", "SuDung1000LanPhaoHoaQuocKhanhÁìÀñÆ·", tbTranslog})
	...
	elseif (self:CheckTask(4, 1500,"","==") == 1) then
		...
		local tbTranslog = {strFolder = %strQK_TranslogFolder, nPromID = %nQK_PromotionID, nResult = 1}
		tbAwardTemplet:Give(tb1500Award, 1, {"Event_QuocKhanhVN", "SuDung1500LanPhaoHoaQuocKhanhÁìÎïÆ·", tbTranslog})
	end	
	local tbTranslog = {strFolder = %strQK_TranslogFolder, nPromID = %nQK_PromotionID, nResult = 1}
