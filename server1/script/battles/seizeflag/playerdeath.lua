...
if (PlayerIndex1 > 0) then
	...
	if (GetCurCamp() ~= deathcamp) then
		...
		if (TAB_SERIESKILL[launchrank][currank] == 1) then
			...
			if (mod(serieskill_r, 3) == 0) then
				if (deathcamp == 1) then
						...
						Msg2Player("<color=yellow>你获得三连斩积分"..BT_GetTypeBonus(PL_MAXSERIESKILL, 2))
				else
						bt_addtotalpoint(BT_GetTypeBonus(PL_MAXSERIESKILL, 1))
						Msg2Player("<color=yellow>你获得三连斩积分"..BT_GetTypeBonus(PL_MAXSERIESKILL, 1))
				end
			end
		end
		...
		if (GetCurCamp()  == 1) then
			str  = "宋方"..launrankname..LaunName.." 杀死了金方"..rankname..DeathName.."，PK人数增为"..BT_GetData(PL_KILLPLAYER);
		else
			str  = "金方"..launrankname..LaunName.." 杀死了宋方"..rankname..DeathName.."，PK人数增为"..BT_GetData(PL_KILLPLAYER);
		end
		
		Msg2Player("<color=pink>恭喜：你杀死了敌方:"..rankname..DeathName.." PK人数增为"..BT_GetData(PL_KILLPLAYER));
