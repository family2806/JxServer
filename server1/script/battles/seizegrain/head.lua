...
function sf_addgrain(camp, x, y)
	...
	local npcidx = AddNpc(tnpcid, 1, SubWorld, x, y, 1, "粮食包");
	...
	if (GetMissionV(MS_STATE) == 2 and BT_GetData(PL_PARAM3) ~= 0) then
		--该玩家现在正处于背粮状态，死亡里需要让粮食包掉下来
			...
			if (GetCurCamp() == 1 ) then
				Msg2MSAll(MISSIONID, format("<color=0x00FFFF>宋方<color=yellow>%s<color> 失去粮食包，在<color=yellow>%d,%d",GetName(),floor(x/8),floor(y/16)))
			else
				Msg2MSAll(MISSIONID, format("<color=0x9BFF9B>金方<color=yellow>%s<color> 失去粮食包，在<color=yellow>%d,%d",GetName(),floor(x/8),floor(y/16)))
			end
	end
...
--by zero 2007-8-1 获得所有赢方
	...
	for i=1,getn(Win_Players_Table) do
		...
		if(player_total_point >= 1000 and JG_T_Value < jg_ndate) then
			...
			Msg2Player("您获得宋金勋功牌")
			WriteLog(format("[宋金战场]\t%s\tName:%s\tAccount:%s\t 1000 个积分以上将收到一个勋功牌",
...
function GameOver()
	...
	if (bonus_s > bonus_j ) then
		resultstr = resultstr.."宋金运粮模式已结束,宋方和金方粮包的数量为"..BT_GetGameData(GAME_CAMP1)..":"..BT_GetGameData(GAME_CAMP2)..", 宋方取得胜利!";
		Msg2MSAll(MISSIONID, resultstr)
		WriteLog(resultstr.."双方人数比为: 宋"..GetMSPlayerCount(MISSIONID, 1)..": 金 "..GetMSPlayerCount(MISSIONID, 2)..". 结束时间"..date("%H:%M"))
		BT_ReportResult(1)
		sf_winbouns(1)
		RecordBTCount_Win(1)		--在每个人任务变量里记录它此次宋金战场胜负状况
	elseif (bonus_s < bonus_j) then
		resultstr = resultstr.."宋金运粮模式已结束,宋方和金方粮包的数量为 "..BT_GetGameData(GAME_CAMP1)..":"..BT_GetGameData(GAME_CAMP2)..", 金方取得胜利!"
		Msg2MSAll(MISSIONID, resultstr)
		WriteLog(resultstr.."双方人数比为: 宋"..GetMSPlayerCount(MISSIONID, 1)..": 金 "..GetMSPlayerCount(MISSIONID, 2)..". 结束时间"..date("%H:%M"))
		BT_ReportResult(2)
		sf_winbouns(2)
		RecordBTCount_Win(2)		--在每个人任务变量里记录它此次宋金战场胜负状况
			...
			resultstr = resultstr.."宋金运粮模式结束，结果为打平,宋方和金方粮包的数量为 "..bonus_s.."";
			Msg2MSAll(MISSIONID, resultstr);
			WriteLog(resultstr.."双方人数比为: 宋"..GetMSPlayerCount(MISSIONID, 1)..": 金 "..GetMSPlayerCount(MISSIONID, 2)..".结束时间"..date("%H:%M"))
			local award_over = BATTLES_TIEGAME_POINT * bt_getgn_awardtimes()
			bt_camp_getbonus(1, award_over, "本次宋金战斗战平了！将士暂且收兵回营,养兵蓄锐,来日与敌人再绝雌雄! (得到奖励<color=yellow>"..award_over.."<color> 积分) ", 75);
			bt_camp_getbonus(2, award_over, "本次宋金战斗战平了！将士暂且收兵回营,养兵蓄锐,来日与敌人再绝雌雄! (得到奖励<color=yellow>"..award_over.."<color> 积分) ", 75);
	end;
...
function sf_join(camp)
	...
	if (GetMSPlayerCount(MISSIONID, camp) >= MAX_MEMBERCOUNT ) then
		if (camp==1) then
			Msg2Player("以我军现在的兵力已足以全灭金兵，壮士请等下次战斗吧!")
			return
		elseif (camp==2) then
			Msg2Player("以我军现在的兵力已足以全灭金兵，壮士请等下次战斗吧!")
			return
...
if (camp == 1) then
	...
	str = GetName().."加入宋方"
	EnterChannel(PlayerIndex, "宋方"..szGAME_GAMELEVEL[level].."运粮")
else
	posx = GetMissionV(MS_HOMEIN_X2)
	posy = GetMissionV(MS_HOMEIN_Y2)
	str = GetName().."加入金方"
	EnterChannel(PlayerIndex, "金方"..szGAME_GAMELEVEL[level].."运粮")
end
...
function sf_addgrain_point(ncamp)
	...
	if (ncamp == 1) then
		...
		Msg2MSAll(MISSIONID, "宋方 <color=yellow>"..GetName().."<color>护送成功一粮包");--左下角的即时战况："GetCamp方玩家GetName()成功运送帅旗到达目的地！"
	elseif(ncamp == 2) then
		BT_SetGameData(GAME_CAMP2, BT_GetGameData(GAME_CAMP2) + 1);
		SetMissionV(MS_TOTALGRAIN_J, GetMissionV(MS_TOTALGRAIN_J) + 1);
		Msg2MSAll(MISSIONID, "金方<color=yellow>"..GetName().."<color>护送成功一粮包");--左下角的即时战况："GetCamp方玩家GetName()成功运送帅旗到达目的地！"
	end;
