...
--	--模式总排行榜
	...
	if (GetMissionV(MS_MOSTDEATH) == 0) then
		if (bonus_s > bonus_j ) then
			resultstr = resultstr.."宋金战斗已结束，最终宋金双方获得的总积分为"..bonus_s.."："..bonus_j.."，宋方赢得了胜利！";
		...
		elseif (bonus_s < bonus_j) then
			resultstr = resultstr.."宋金战斗已结束，最终宋金双方获得的总积分为"..bonus_s.."："..bonus_j.."，金方赢得了胜利！"
			Msg2MSAll(MISSIONID, resultstr)
			WriteLog(resultstr)
			BT_ReportResult(2)
			...
			bt_camp_getbonus(1, award_over, "本次宋金战斗战平了！将士暂且收兵回营,养兵蓄锐,来日与敌人再绝雌雄！<color=yellow>", 75);
			bt_camp_getbonus(2, award_over, "本次宋金战斗战平了！将士暂且收兵回营,养兵蓄锐,来日与敌人再绝雌雄！<color=yellow>", 75);
			sf_winbouns(0)
			WriteLog(resultstr)
		end;
...
if (camp == 1) then
	...
	posy = GetMissionV(MS_HOMEIN_Y2)
		str = GetName().."加入了金方阵营"
		EnterChannel(PlayerIndex, "金方"..szGAME_GAMELEVEL[level].."杀戮模式")
end
...
function but_pop2home( camp )
	...
	if ( count > 0 ) then
		...
		for i = 1, count do
			...
			SetFightState(1)
			Msg2Player("<color=pink>你不能继续停留在后营")
		end
	end
	...
	if ( count_remain > 0 ) then
		for i = 1, count_remain do
			...
			if( remain_time > 0) then
				Msg2Player("<color=pink>你还能在后营停留"..remain_time.."秒")
			end
		end
	end
	PlayerIndex = oldPlayerIndex
