...
function AddTeTan(nNowTimeCount)
	if tbAppearTime[nNowTimeCount - RUNGAME_TIME] == 1 then
		...
		Msg2MSAll(MISSIONID, "ĞÂµÄ§Æc Th¸mÒÑ¾­½øÈëÕ½³¡ÁË£¬´ó¼Ò¿ìµãĞĞ¶¯!");
		...
		for i=1,4 do
			...
			local msg = format("Ôö¼ÓËÎ½ğ§Æc Th¸m: X = %d Y = %d",tbPosition_s[nIndex].x,tbPosition_s[nIndex].y)
			WriteLog(msg)
			tbNpc.szName = "§Æc Th¸m"
			basemission_CallNpc(tbNpc[2], nNowMapId, tbPosition_j[nIndex].x * 32, tbPosition_j[nIndex].y * 32)
			local msg = format("Ôö¼Ó½ğ¾ü§Æc Th¸m: X = %d Y = %d",tbPosition_j[nIndex].x,tbPosition_j[nIndex].y)
			WriteLog(msg)
		end
	end
end
...
function OnTimer()
	...
	if (t >= RUNGAME_TIME) then
		...
		if (t > RUNGAME_TIME) then
			...
			if (mod(t, 3) == 0 ) then
			msstr = "¹«¸æ:µ±Ç°ËÎ½ğË«·½×Ü»ıphótÎª"..GetMissionV(MS_TOTALPOINT_S)..":"..GetMissionV(MS_TOTALPOINT_J);
