...
-------------------------------------------------------------------------
	...
	bt_add_a_diagnpc(FILE_TRANSPORT1, TNPC_TRANSPORT1, x * 32 ,y * 32 , "宋军传送员");


	transportxy = GetIniFileData(mapfile, "Area_"..j_area, "transportxy");
	x,y = bt_str2xydata(transportxy)
	bt_add_a_diagnpc(FILE_TRANSPORT2, TNPC_TRANSPORT2, x * 32, y * 32, "金军传送员");
	...
	local str1 = lvlstr.."宋金战役之杀戮模式一触即发，目前正进入报名阶段，欲参战者请尽快从襄阳或朱仙镇进入，或使用宋金诏书进入宋金战场报名点报名,报名剩余时间:"..RestMin.."分"..RestSec.."秒。参战条件:等级不小于40级，报名费"..BATTLES_JOINFEE.."两。";
	--AddGlobalNews(str1);
	CreateChannel("宋方"..szGAME_GAMELEVEL[level].."杀戮模式", 9)
	CreateChannel("金方"..szGAME_GAMELEVEL[level].."杀戮模式", 10)

	BT_SetMissionName("杀戮模式")
	BT_SetMissionDesc("背景：公元1160年，金主完颜亮为实现灭宋大计，积极集结大军，酝酿南下。金军频频南侵，首当其冲的便是南宋重镇襄阳，一场艰苦卓绝的襄阳防卫战即将展开。<enter><enter><color=yellow>杀戮模式：在战场上，尽可能多的杀死敌方玩家，为本方赢得积分。积分多的一方获胜。<enter>每个玩家在后营停留的时间不能超过 2 分钟，战场内没有战斗NPC。")
...
function EndMission()
	...
	DeleteChannel("金方"..szGAME_GAMELEVEL[level].."杀戮模式")
...
function OnLeave(RoleIndex)
	...
	if (GetCurCamp() == 1) then
		...
		LeaveChannel(PlayerIndex, "金方"..szGAME_GAMELEVEL[level].."杀戮模式")
	end
	sf_onplayerleave()
