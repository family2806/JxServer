...
local _PublicRank = function(nId)
	...
	if szName ~= nil and szName ~= "" then
		if nId == 1 then
			szMsg = format("恭喜%s %s 已获得第一，帮会成员够条件可以领取，帮会成员够条件可以领取%u 经验",szName, "此次排名", 6e7)
		elseif nId == 2 then
			szMsg = format("恭喜%s %s 已获得第一，帮会成员够条件可以领取%u 经验",szName, "每周排名", 15e7)
		elseif nId == 3 then
			szMsg = format("恭喜%s %s 已获得第一，帮会成员够条件可以领取%u 经验",szName, "整个活动排名", 20e7)
		end
	end
